!(function (win, plugin) {
    ...
    plugin.saveCallBack = function (mainConfig, controlConfig, page) {
        ...
        var nodeJson = paramsCfg.params.nodeJson;
        var node = oui.parseJson(nodeJson);
        var postData = $.extend(true,page,node);
        var postUrl = paramsCfg.params.saveUrl;
        postUrl = oui.setParam(postUrl,'id',postData.id);
