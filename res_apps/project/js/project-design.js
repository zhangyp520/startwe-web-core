!(function (win, $) {
    ...
    var ProjectController4Design = {
        "package": "com.startwe.models.project.web",//com.startwe.models.project.web.ProjectController4Design
        ...
        init:function(){
            ...
            var urlParams = oui.getParam('urlParams');
            if(!urlParams){
                urlParams = window.urlParams||{};
            }
            urlParams = oui.parseJson(urlParams); 
            ...
            me.menuActionEnum = {
                ...
                viewDesignPage:{//呈现模板设计
                    action:function(node,treeMap,orgGraph){
                         var id = node.id;

                 
                         orgGraph.showViewDesignPage(id);
                ...
                designPortal:{//门户设计页面
                    action:function(node,treeMap,orgGraph){
                        // console.log("点击门户设计按钮")
                        // var id = node.id;
                        // orgGraph.showPortalDesign(id);
                        console.log('designPortal',com.startwe.models.project.web.ProjectController4Design.data.queryPortalsByProjectIdUrl)
                ...
                menuDesign:{//菜单设计页面
                    action:function(node,treeMap,orgGraph){
                        var id = node.id;
                        orgGraph.showMenuDesign(id);
                    }
                },
                powerDesign:{
                    action:function(node,treeMap,orgGraph){
                        var id = node.id;
                        orgGraph.showPowerDesign(id);
                    }
                },
                designQueryPage:{
                    //设计查询页面
                ...
                exportPhoneApp:{
                    action:function(node,treeMap,orgGraph){
                        var exportUrl = me.data.exportPhoneAppUrl;
                        exportUrl = oui.addParams(exportUrl,{
                            userId:oui.cookie('userId'),
                            tokenId:oui.cookie('tokenId'),
                            nodeId:node.node.id,
                ...
                addLogic:{
                    ...
                    action:function(node,treeMap,orgGraph,target,result,res){
                        ...
                        treeMap.addNode(newNode); //添加节点 
                        console.info(treeMap,'treeMaptreeMap')
                        //刷新当前节点和下面的节点列表
                        orgGraph.refreshByNodeId(node.id,treeMap);
                    }
                },
                ...
                removeAll4ajax:{ //删除当前和所有子孙,并执行后台api删除
                    ...
                    action:function(node,treeMap,orgGraph,target,result){
                        console.log('removeAll4ajax', this)
                        if(!node.parentId){
                            return ;
                        }
                        var parentId = node.parentId;
                        treeMap.currentNodeId = '';
                        var $groupButton = $('.node-group-buttons');
                        var view = oui.getById('project-design');
                        var html4groupButton = view.getHtmlByTplId('node-group-buttons-tpl',{
                            ProjectController4Design:ProjectController4Design
                        });
                        $groupButton[0].outerHTML = html4groupButton;
        ...
        showListPageDesign:function(id){

            console.info('showListPageDesign',id)
            var me  = this;
        ...
        showPageDesign:function(id){
            ...
            oui.getData(me.data.loadPageDesignUrl||loadUrl,{
                ...
                url = oui.addParams(url, {
                    pageId:id,
                    queryPageViewsByProjectIdUrl:me.data.queryPageViewsByProjectIdUrl,
                    projectId:me.data.project.id,
                ...
                var  params = {
                    designTplUrl: "/res_engine/page_design/common/runtime/page-design-form-tpl.html",
                    uploadUrl:oui.uploadURL,//跨静态页面传值，用于上传组件
                    /**必须参数*/
                    controls: [],//已有的控件列表
                    mainTemplate: '',//页面的业务属性面板模板获取方法
                    bizJs:[oui.getContextPath()+'res_apps/project/js/page-design/page-design-plugin-1.6.js'],
                    saveCallBack:'saveCallBack',
                    /**非必须参数*/
                    viewType:'openWindow',
                    buttons: "preview,save,merge,split,insertColumn4prev,insertRow4prev,removeColumn,removeRow", //按钮参数,如save,print 等按钮事件名
                    ...
                    bizCss: [],
                    canCloneControl:true,//是否可以复制控件
                    pageTypeTree:res.pageTypeTree, 
                    treeMap : me.data.treeMap||{},
                    apiMap:me.data.apiMap,
                    //其它业务参数调用传递,重要提示：cfg.params的参数最好使用简单map对象，不能包含任何dom对象或window对象等
                    params: {
                        nodeJson :oui.parseString(node),
                        queryPageModelsUrl:queryPageModelsUrl,//查询页面模型列表的url
                        ...
                        queryPageFieldsAndAssociationFieldsUrl:queryPageFieldsAndAssociationFieldsUrl,//查询直接关联和间接关联的字段的url
                        //逻辑树数据接口
                        loadLogicMenuUrl:me.data.loadLogicMenuUrl,
                        loadLogicUrl:me.data.loadLogicUrl,
                        saveDesignUrl :me.data.saveDesignUrl, //保存树结构
                        saveUrl:  me.data.savePageDesignUrl|| oui.biz.Tool.getApiPathByController(me.FullName.replace('4Design',''),'savePageDesign'),
                        //saveUrl:  "http://192.168.0.111:8085/com.startwe.models.project.web.ProjectListTemplateController.saveListTemplate.biz",
                        queryFlowsUrl: queryFlowsUrl,
                        omUrlMap:me.data.omUrlMap
                    }
                } 
                ...
                oui.openWindow({
                    url: url,
                    // openType: '_blank',
                    params:{scrollbars:'no',resizable:'no',scroll:'no',location:'no',top:0,bottom:0},
        ...
        //页面呈现设计    L.T
        showViewDesignPage:function(id){
       
                    var me = this; 
                    console.info(id,'id');
                    var loadUrl =  oui.biz.Tool.getApiPathByController(me.FullName.replace('4Design',''),'loadPageDesign');

                    console.info(me.data.loadPageDesignUrl,'me.data.loadPageDesignUrl');
                    ...
                    oui.getData( me.data.loadPageDesignUrl ||loadUrl,{
                        ...
                        var url = oui.getContextPath() + 'res_engine/page_list_design/pc/page_list_design.html';
                        url = oui.addParams(url, {
                            pageId:id,
                            queryPageViewsByProjectIdUrl:me.data.queryPageViewsByProjectIdUrl,
                            projectId:me.data.project.id,
        ...
        // 门户设计
        showPortalDesign:function(id){
            var me  = this;

            var url = oui.getContextPath()+'index4vue.html#';
            var portalUrl = oui.getContextPath()+'res_engine/portal_design/portal-design-list.vue.html' ;
            portalUrl = oui.addParams(portalUrl,{
                queryPortalsByProjectIdUrl:me.data.queryPortalsByProjectIdUrl,
                circleId:me.data.project.circleId,
                projectId:me.data.project.id
            ...
            console.log(url,'门户设计地址')

        },
        // 菜单设计
        showMenuDesign:function(id){
            var me  = this;
            var url = oui.getContextPath()+'index4vue.html#';
            var menuUrl = oui.getContextPath()+'res_engine/menu/menu-design-list.vue.html' ;
            menuUrl = oui.addParams(menuUrl,{
                queryMenusByProjectIdUrl:me.data.queryMenusByProjectIdUrl,
                circleId:me.data.project.circleId,
                projectId:me.data.project.id
            });
        ...
        //查询节点的背景和字体样式
        findNodeStyle: function (nodeId) {
            var me = this;
            var treeMap = me.data.treeMap;
            var node = treeMap.findSourceNode(nodeId);
            var otherAttrs = node.otherAttrs || {};
            var textColor = otherAttrs.textColor || "";
            var borderColor = otherAttrs.borderColor || "";
            var bgColor = otherAttrs.bgColor || "";
            var strStyle ='color:' +textColor +';'+ 'background-color:' + bgColor + ';border-color:' +borderColor +';';
            return strStyle;
        },
        //查询节点的图片样式
        findNodeIcon: function (nodeId) {
            var me = this;
            var treeMap = me.data.treeMap;
            var node = treeMap.findSourceNode(nodeId);
            var otherAttrs = node.otherAttrs || {};
            //图片id为空的时候 为默认图片  默认图片根据节点类型来进行处理
            if (!otherAttrs.iconId) {
                var config = {
                    project: {
                        name: 'project',
                        imgUrl: '/res_startwe/assets/project.png'
                    },
                    module: {
                        name: 'module',
                        imgUrl: '/res_startwe/assets/module.png'
                    },
                    app: {
                        name: 'app',
                        imgUrl: '/res_startwe/assets/app.png'
                    },
                    page: {
                        name: 'page',
                        imgUrl: '/res_startwe/assets/page.png'
                    },
                    logic: {
                        name: 'logic',
                        imgUrl: '/res_startwe/assets/logic.png'
                    }
                };
                return config[node.nodeType] ? config[node.nodeType].imgUrl : "";
            } else {//当图片id不为空的时候
                if (otherAttrs.iconId.indexOf("default") > -1) {
                    var split = otherAttrs.iconId.split("_");
                    var imgName = '/res_startwe/assets/' + split[1] + '.png';
                    return imgName;
                }else {
                    var iconData = otherAttrs.iconData || [];
                    var one = oui.findOneFromArrayBy(iconData, function (item) {
                        if (item.id == otherAttrs.iconId) {
                            return true;
                        }
                    });
                    return one ? one.imgUrl : "";
                }
            }
        },


        iconsClick:function(cfg){
            // alert(111)
            cfg.e.stopPropagation&&cfg.e.stopPropagation();
            // oui.follow4fixed(cfg.el,$('.node-icon-seting')[0],true);
            var me = this;
            var nodeId = $(cfg.el).attr('node-id');
            var view = oui.getById('project-design');
            $('.allowedDrop','.orgchart').removeClass('allowedDrop');
            $('.second-menu').remove();
            var html = view.getHtmlByTplId('node-icon-tpl',{
                nodeId:nodeId,
                treeMap:me.treeMap
            });
            $('.orgchart').closest('.project-design-graph').append(html);

            me.data.treeMap.currentNodeId=nodeId;
            oui.parse({
                container: $('.orgchart').closest('.project-design-graph')[0],
                callback:function(){
                    oui.follow4fixed($(cfg.el).closest('div.node')[0],$('.node-icon-seting')[0],true);
                }
            }); 
            $('.node.current-node').removeClass('current-node');
            $(cfg.el).closest('div.node').addClass('current-node');

            var controller = $('.project_menu').attr('oui-controller');
            controller = oui.parseJson(controller);
            if(controller){
                controller.buildTreeData && controller.buildTreeData();
            }
            var $groupButton = $('.node-group-buttons');
            var html4groupButton = view.getHtmlByTplId('node-group-buttons-tpl',{
                ProjectController4Design:ProjectController4Design
        ...
        event2menuAction:function(cfg){
            ...
            if(currActionEnum.api){
                ...
                var apiMap = me.data.apiMap;
                if(currActionEnum.api != 'removeAllNode'){
        ...
        load:function(param,callback){
            ...
            console.info(path,'pathpathpathpath')
            oui.getData(path,param,function(res){
                if(res.success){
                    var project = res.project||{};
                    me.data.project = project;
                    me.PortalController.data.project = project;
                    if(res.designJson){
                        me.treeMap = me.newTreeMap(oui.parseJson(res.designJson),'id','parentId','name');
                        me.treeMap.map[me.treeMap.rootIds[0]].node.name=me.data.project.name;
                    }else{
                    ...
                    me.data.treeMap.currentNodeId = project.id;
                    ...
                    me.data.exportPhoneAppUrl = res.exportPhoneAppUrl;//导出移动端应用
                    ...
                    me.data.queryPortalsByProjectIdUrl = res.queryPortalsByProjectIdUrl;//查询门户列表api
                    me.data.queryMenusByProjectIdUrl = res.queryMenusByProjectIdUrl;//查询菜单列表api
                    me.data.queryPageViewsByProjectIdUrl = res.queryPageViewsByProjectIdUrl//列表模板 
                    
                    me.data.queryConfigUrl = res.queryConfigUrl; //查询项目配置信息
                    me.data.saveConfigUrl = res.saveConfigUrl; //保存项目配置信息

                    //逻辑树数据接口
                    me.data.loadLogicMenuUrl = res.loadLogicMenuUrl
                    callback&&callback();
