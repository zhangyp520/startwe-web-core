...
(function(win){
	...
	var getHtml = function(){
		...
		if(!clazz._getHtml[right+this.attr('showType')]){// 缓存编译函数
			...
			if(temp){
				clazz._getHtml[right+this.attr('showType')] = template.compile( temp); //编译模板并 缓存模板渲染函数
			}
		}
		var map = this.getMap();
		...
		if(right =='design' || right=='disabled'){
			...
			h = clazz._getHtml[right+this.attr('showType')](map);
		...
		if(hasRelationBtnValue){
			if(!clazz._getHtml['readOnly'+this.attr('showType')]){// 缓存编译函数
				...
				if(temp){
					clazz._getHtml['readOnly'+this.attr('showType')] = template.compile( temp); //编译模板并 缓存模板渲染函数
				}
			}
			...
			if(this['getHtml4readOnly']){ //如果存在自定义 right类型则根据right类型执行函数渲染对应状态
				...
				rehtml=  clazz._getHtml['readOnly'+this.attr('showType')](map);
			}else{
				//只读状态只输出文本内容
				//对于浏览态的控件 1、如果传入了valueObj的display属性则显示控件内容
				//2、如果没有传入valueObj则使用默认value显示控件内容
	...
	/** 创建模板方法 定义****/
	OuiFormControl.buildTemplate = BaseControl.buildTemplate;
