...
(function (oui) {
    ...
    var MapConfig = {
        baseUrl: "http://js.arcgis.com/3.12/",
        TileServer4LabelMap: 'http://1.85.55.27:8080/ServiceSystem2016/rest/service/SxImgLabelMap/r9qECrGPCxQNvMEu/TileServer',
        TileServer4Map: 'http://1.85.55.27:8080/ServiceSystem2016/rest/service/SxImgMap/Z97xdPWqZrTeXSd9/TileServer'
    };
            ...
            if (!oui._mapReadyState) {
                ...
                if (!document.getElementById("mapJsTag")) {
                    ...
                    oui.require4notSort([
                        ...
                        MapConfig.baseUrl + "esri/css/esri.css",
        ...
        XYMapApi.prototype = {
            ...
            initMap: function () {
                ...
                require([
                    ...
                    var oolayer = new ArcGISTiledMapServiceLayer(XYMapApi.TileServer4LabelMap);
                    var olayer = new ArcGISTiledMapServiceLayer(XYMapApi.TileServer4Map);

                    // self.map.addLayer(basemap);//全球底图
                    // self.map.addLayer(annolayer);//全球标注图
                    self.map.addLayer(olayer);//西安底图
                    self.map.addLayer(oolayer);//西安标注图
            ...
            geolocation: function (callback) {
                ...
                callback && callback({
                    ...
                    address: "经度：" + center.lng + "；纬度：" + center.lat
                });
            },
            /**
             * 创建marker
            ...
            setZoomAndCenter: function (level, position) {
                this.map.centerAndZoom(new esri.geometry.Point(Number(position.lng), Number(position.lat), this.spr), level);
            },
            /**
             * 设置标点聚合
             */
