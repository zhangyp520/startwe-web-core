(function (win) {
    ...
    ImageMulti.templateHtml[0] =
        ...
        '<img src="{{oui.getImgUrl(item.url,320,320)}}"/>' +
        '</p>' +
        '<label {{if (right!="design")}}onclick="return false;" ontap="oui.getByOuiId({{ouiId}}).multiSelectClick(this);"{{/if}}>' +
        '<div class="multiSelect-wrapper">' +
        '<input type="checkbox" id="imagemulti_{{id}}_{{index}}" name="{{name}}" ' +
    ...
    var multiSelectClick = function (el) {
        ...
        var $obj = $(el);
        var $el = $obj.find("input[type='checkbox']");
        el = $el[0];
        ...
        if (el.checked) {
            ...
            _$p.removeClass("images-border-active");
        }
