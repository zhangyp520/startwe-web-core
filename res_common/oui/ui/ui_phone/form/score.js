...
(function (win) {
     ...
     */
    Score.templateHtml = [];
    Score.templateHtml[0] =
        '<input id="{{id}}" name="{{name}}" validate="{{validate}}" type="hidden" value="{{value}}" />' +
        '<ul>' +
        '{{each _starsArr as item index}}' +
        '<li {{if value > index}} class="oui-score-active" {{/if}} ontap="oui.getByOuiId({{ouiId}}).setScore({{index+1}});" ></li>' +
        '{{/each}}' +
        '<ul>';
    ...
    var setScore = function (v) {
        ...
        //var $el = $(this.getEl());
        //$el.find("li").removeClass("oui-score-active");
        //$el.find("li").each(function (i) {
        //    if ((i + 1) <= v) {
        //        $(this).addClass("oui-score-active");
        //    }
        //});
    };
    /*******************************控件类的自定义函数 end******************************************/
