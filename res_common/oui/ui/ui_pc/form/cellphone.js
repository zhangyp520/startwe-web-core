...
(function(win){
    ...
    var CellPhone = function(cfg) {
        ...
        this.attrs = this.attrs + ',checkMsgCode,placeholder';
    ...
    CellPhone.templateHtml[0] ='<input id="{{id}}" placeholder="{{placeholder}}" class="oui-form" '+
        ...
        '<div class="oui-cellphone-check" style="{{oui.getByOuiId(ouiId).isCheckMsgCode()?"":"display:none"}}">'+
        '<input id="checkMsgCode{{id}}" class="check-msg-code" ' +
        '{{if right&&(right=="design")}}disabled="disabled" '+
        '{{/if}}'+
        'validate="{{checkMsgCodeValidate}}" value="{{code}}" onblur="oui.getByOuiId({{ouiId}}).updateCode(this);" onpropertychange="oui.getByOuiId({{ouiId}}).updateCode(this);" onkeyup="oui.getByOuiId({{ouiId}}).updateCode(this);" placeholder="请输入手机验证码"/>'+
    ...
    //otherAttrs: checkMsgCode,cellphones
    /***********************************控件事件***********************************/
    ...
    var showVerify = function(sendBtnEl){
        ...
        oui.showVerifyHtml({
            container:el, //指定显示容器
            product:'bind',// 极验显示方式,详情参考极验官网文档
            userId:this.attr('value'), //极验校验需要的tokenId，默认不传入，根据业务场景配置
            onReady:function(){ //图片验证区域显示完成后事件
                /*** bind 模式自动显示 图片校验区域***/
                var o = oui.getVeriyObject();
                o&& o.verify();
            },
            onClose:function(){//图片验证区域关闭事件
                /** 关闭时清除dom区域***/
                oui.destroyVerify();
            },
            onSuccess:function(validateResult){//成功校验触发
