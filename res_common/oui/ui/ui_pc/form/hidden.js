(function(win){
	 ...
	 */
	Hidden.templateHtml=[]; 
	Hidden.templateHtml[0] ='<input id="{{id}}" class="oui-form" name="{{name}}" validate="{{validate}}"  type="hidden" value="{{value}}"/>' ;
	Hidden.templateHtml[1] ='<input id="{{id}}" {{if right&&(right=="design")}}disabled="disabled" '+
		'{{/if}}'+
