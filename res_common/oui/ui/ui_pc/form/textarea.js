(function(win){
	...
	var Textarea = function(cfg) {
		...
		this.attrs =this.attrs+',rows,placeholder,isReadOnly,onclick,isAppend';
		this.validate = validate;
		this.init = init;
		this.blur2validate = blur2validate;
		this.click = click;
	...
	Textarea.templateHtml[0] ='<textarea id="{{id}}" rows="{{rows}}" '+
	...
	'name="{{name}}" ' +
		...
		'onclick="oui.getByOuiId({{ouiId}}).click();" ' +
		'class="oui-form" validate="{{validate}}" oninput="oui.$.ctrl.ouiformcontrol.change({{ouiId}},this);" onpropertychange="oui.$.ctrl.ouiformcontrol.change({{ouiId}},this);" onblur="oui.$.ctrl.ouiformcontrol.blur({{ouiId}},this);oui.getByOuiId({{ouiId}}).blur2validate(this);" >{{=value}}</textarea>' ;
	...
	Textarea.templateHtml4readOnly[0] ='\
		<div class="form-collapse-item">\
			...
			<div class="collapse-more display_none">\
				<i title="查看更多" class="collapse-btn-info" onclick="oui.getByOuiId({{ouiId}}).showOrHideMore4ReadOnly();"></i>\
			</div>\
		</div>\
		';
    ...
    var click = function(){
		...
		if(isAppend){
			...
			var me = this;
			var textAreaDialog = oui.getTop().oui.showInputDialog( title||'追加文本', function(result){
				if(result){
					if(value){
						value = value+'\n' ;
					}
			...
			$(textAreaDialog.getEl()).find('textarea').focus();
		}
	};
