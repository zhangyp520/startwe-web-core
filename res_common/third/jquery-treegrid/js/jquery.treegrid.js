...
(function($) {
    ...
    var methods = {
                ...
                arr.push({
                    ...
                    run:function(){
                        ...
                        window.setTimeout(function(){
                            methods.queue2RunTree(curr.$el,settings);
                            curr.next();
                        },10);
                    },
                    next:function(){
        ...
        initTree4MultiData:function(options){
            var settings = $.extend({}, this.treegrid.defaults, options);
            var arr = [];
            var _self = this;
            var obj = this.each(function(i) {
                ...
                arr.push({
                    ...
                    run:function(){
                        var curr = this;
                        window.setTimeout(function(){
                            _self.queue2RunTree(curr.$el,settings);
                            curr.next();
                        },1);
                    },
                    next:function(){
        ...
        queue2RunTree:function($el,settings){
            $el.treegrid('setTreeContainer', $el);
            $el.treegrid('setSettings', settings);
            settings.getRootNodes.apply($el[0], [$el]).treegrid('initNode', settings);
        },
        /**
         * Initialize node
         * 
