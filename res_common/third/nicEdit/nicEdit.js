...
var fontSizes = {1 : '12', 2 : '14', 4 : '16', 5 : '18', 6 : '20', 7 : '24'}; //3号size无法使用
...
var nicEditorInstance = bkClass.extend({
	...
	construct : function(e,options,nicEditor) {
		...
		if(isTextarea || this.options.hasPanel) {
			...
			//$(this.editorContain).attr('id','editorContain');
		...
		$(this.editorContain).height($(this.editorContain).height());
	...
	selected : function(e,t) {
		...
		//editor 值改变事件 selected 
		//在键盘输入时 可以执行内容改变的监听
		NicEditPlugin.change2updateLinkColor();
	...
	nicCommand : function(cmd,args) {
		...
		/**自定义插件扩展 **/
		nicCommandPlugin[cmd] && nicCommandPlugin[cmd](args,this);
		NicEditPlugin.change2updateLinkColor();
		//此处是执行命令的根源 改变富文本样式//可以在此处执行属性同步改变监听
		this.onChange&&this.onChange(); 
	}		
});
/**
 * 自定义插件 方法方便扩展 NICEditor
 * @type {{change2updateLinkColor: Function}}
 */
var NicEditPlugin = {
	change2updateLinkColor:function(){
		$('font[color]').each(function(){$(this).find('a').css({color:$(this).attr('color')})}); //解决 a标签输入的样式问题
	}
}
/**
 * 命令执行插件函数配置
 * @type {{foreColor: Function, justifyleft: Function, justifycenter: Function, justifyright: Function}}
 */
var nicCommandPlugin = {
	'foreColor':function(args,editor){
		if(args =='#000000'){
			args = "#000001";
		}
		document.execCommand('foreColor',false,args);
		$(editor.getElm()).find('font[color=#000001]').each(function(){
			$(this).attr('color','#000000');
		});
	},
	'fontsize':function(args,editor){
		$(editor.getElm()).find('font[size='+args+']').each(function(){
			$(this).css('font-size',fontSizes[args]+"px");
		});
	},
	'justifyleft':function(args,editor){
		if(typeof oui.browser.ie =='undefined'){
			return ;
		}
		$(editor.getElm()).find('[align=left]').each(function(){
			$(this).css('text-align','left');
			$(this).attr('align','');
		});
	},
	'justifycenter':function(args,editor){
		if(typeof oui.browser.ie =='undefined'){
			return ;
		}
		$(editor.getElm()).find('[align=center]').each(function(){
			$(this).css('text-align','center');
			$(this).attr('align','');
		});
	},
	'justifyright':function(args){
		if(typeof oui.browser.ie =='undefined'){
			return ;
		}
		$(editor.getElm()).find('[align=right]').each(function(){
...
var nicEditorPane = bkClass.extend({
	construct : function(elm,nicEditor,options,openButton) {
		...
		//$(this.contain).css('background','red');
...
var nicEditorAdvancedButton = nicEditorButton.extend({
	...
	addForm : function(f,elm) {
		...
		new bkElement('input').setAttributes({'type' : 'submit',value:'确认'}).setStyle({backgroundColor : '#efefef',border : '1px solid #ccc', margin : '3px 0', 'float' : 'left', 'clear' : 'both'}).appendTo(this.form);
...
var nicButtonTips = bkClass.extend({
	...
	create : function(button) {
		...
		/*
		 *if(oui&&oui.showTips){
			oui.showTips({content:button.options.name,el:button.button,left:10,top:2});
			return ;
		} 
		 */
		if(!this.pane) {
...
var nicSelectOptions = {
	buttons : {
		'fontSize' : {name : __('字体大小'), type : 'nicEditorFontSizeSelect', command : 'fontsize',hidden:true},
		'fontFamily' : {name : __('字体类型'), type : 'nicEditorFontFamilySelect', command : 'fontname',hidden:true},
		'fontFormat' : {name : __('标题'), type : 'nicEditorFontFormatSelect', command : 'formatBlock',hidden:true}
	}
};
/* END CONFIG */
var nicEditorSelect = bkClass.extend({
	...
	construct : function(e,buttonName,options,nicEditor) {
		...
		this.txt = new bkElement('div').setStyle({overflow : 'hidden', 'float' : 'left', width : '66px', height : '20px', marginTop : '1px', fontFamily : 'sans-serif', textAlign : 'center', fontSize : '12px'}).addClass('selectTxt').appendTo(this.items);
...
var nicEditorFontSizeSelect = nicEditorSelect.extend({
	...
	init : function() {
		...
		for(itm in this.sel) {
			this.add(itm,'<font size="'+itm+'" style="font-size:'+fontSizes[itm]+'px;" >'+this.sel[itm]+'</font>');
		}		
	}
});
...
var nicEditorFontFamilySelect = nicEditorSelect.extend({
	sel : {
		...
		'SimSun':'宋体',
		'NSimSun':'新宋体',
		'FangSong,FangSong_GB2312':'仿宋,仿宋GB2312',
		'KaiTi,KaiTi_GB2312':'楷体,楷体GB2312',
...
var nicEditorFontFormatSelect = nicEditorSelect.extend({
		sel : {'p' : '段落', 'h6' : '标题6', 'h5' : '标题5', 'h4' : '标题4', 'h3' : '标题3', 'h2' : '标题2', 'h1' : '标题1'},
...
var nicLinkOptions = {
	buttons : {
		'link' : {name : '添加链接', type : 'nicLinkButton', tags : ['A'],hidden:true},
		'unlink' : {name : '取消链接',  command : 'unlink', noActive : true,hidden:true}
	}
};
/* END CONFIG */
var copytext_keleyi_com ;
...
var nicLinkButton = nicEditorAdvancedButton.extend({	
	addPane : function() {
		...
		//$(this.ln).attr('target','_blank');
		this.addForm({
			'' : {type : 'title', txt : '编辑链接'},
			'href' : {type : 'text', txt : 'URL', value : 'http://', style : {width: '150px'}},
			'title' : {type : 'text', txt : 'Title'}
			//, 'target' : {type : 'select' ,value:'_blank' , txt : '打开', options : { '_blank' : '新窗口','' : '当前页'},style : {width : '100px'}}
		},this.ln);
		//TODO ..IE 测试
		//console.log(this.ln);
	},
	submit : function(e) {
		...
		this.removePane();
		var secText = getSelectionText();
		if(!secText || $.trim(secText).length==0){
			oui.getTop().oui.alert('请选择连接文字');
			return ;
		}
		if(!this.ln) {
			var tmp = url;
...
var nicImageOptions = {
	buttons : {
		'image' : {name : '添加图片', type : 'nicImageButton', tags : ['IMG'],hidden:true}
	}
