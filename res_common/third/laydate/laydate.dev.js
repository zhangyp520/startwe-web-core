...
win.laydate = function(options){
    options = options || {};
    try{
        as.event = options.event?options.event:( win.event ? win.event : laydate.caller.arguments[0]);
    } catch(e){};
    Dates.run(options);
    return laydate;
};
...
Dates.getPath = (function(){
    ...
    for(var i = 0,len = js.length;i <len ;i++){
        if(js[i].src && js[i].src.indexOf("laydate.dev.js") >= 0){
            jsPath = js[i].src;
            break;
        }
    }
...
Dates.orien = function(obj, pos){
    ...
    if(rect.left + obj.offsetWidth <= Dates.winarea(true)){
        obj.style.left = (rect.left + 1) + "px";
    }else{
        var lefts = rect.left > obj.offsetWidth ? (rect.left + Dates.elem.offsetWidth -  obj.offsetWidth) + 1: Dates.winarea(true) - obj.offsetWidth;
        obj.style.left = Math.max(lefts + (pos ? 0 : Dates.scroll(1)),1) + 'px'; //rect.left + (pos ? 0 : Dates.scroll(1)) + 'px';
    }
    ...
    if(!Dates.box){
        ...
        + Dates.viewtb
          ...
          +'<ul id="laydate_hms">'
            ...
            +'<li><input '+(!Dates.options.timeInput?'readonly':'')+' >:</li>'
            +'<li><input '+(!Dates.options.timeInput?'readonly':'')+' >:</li>'
            +'<li><input '+(!Dates.options.timeInput?'readonly':'')+' ></li>'
          +'</ul>'
...
Dates.checkhms = function(hms){
    for (var i = 0,len = hms.length;i < len;i++){
        var v  = hms[i].value;
        if(v.length <= 0 || v.length > 2 || !(/^\d+$/.test(v))){
            //Dates.msg("时间不合法,请重新输入");
            return false;
        }
        Dates.canClose = true;
    }
        ...
        Dates.on(elem, 'click', function(ev){
            if(!Dates.hasClass(this, as[1])){
                ...
                if(!Dates.options.istime){
                    ...
                    Dates.viewDate(Dates.ymd[0], Dates.ymd[1], this.getAttribute('d')|0);
                }
            }
        });
    });
...
Dates.on(doc,'scroll',function(){
    laydate.reset();
});

laydate.scrollReset = function(dom){
    Dates.on(dom,'scroll',function(){
        laydate.reset();
    });
};
