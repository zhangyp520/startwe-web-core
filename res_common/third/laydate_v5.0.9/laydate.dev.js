    ...
    */
    ...
    Class.prototype.systemDate = function (newDate) {
        ...
        return {
            ...
            , houi: newDate ? newDate.getHours() : 0 //时
    ...
    Class.prototype.checkDate = function (fn) {
        var that = this
            ...
            , checkValid = function (dateTime) {
                ...
                if (dateTime.houi > 23) dateTime.houi = 0, error = true;
                if (dateTime.minutes > 59) dateTime.minutes = 0, dateTime.houi++, error = true;
            ...
            , initDate = function (dateTime, value, index) {
                ...
                lay.each(that.format, function (i, item) {
                    ...
                    if (/yyyy|y/.test(item)) { //年
                        ...
                        dateTime.houi = thisv;
                        options.range && (that[startEnd[index]].houi = thisv);
    ...
    Class.prototype.calendar = function (value) {
        ...
        if (!options.range) that.limit(lay(that.footer).find(ELEM_CONFIRM), null, 0, ['houi', 'minutes', 'seconds']);
    ...
    Class.prototype.list = function (type, index) {
        ...
        if (type === 'year') { //年列表
                ...
                lay(ul).find('ol').each(function (i, ol) {
                    lay(ol).find('li').each(function (ii, li) {
                        that.limit(lay(li), [{
                            ...
                            houi: that[startEnd].houi
                            , minutes: ii
                        }, {
                            houi: that[startEnd].houi
                            , minutes: that[startEnd].minutes
                            , seconds: ii
                        }][i], index, [['houi'], ['houi', 'minutes'], ['houi', 'minutes', 'seconds']][i]);
                    });
                });
                if (!options.range) that.limit(lay(that.footer).find(ELEM_CONFIRM), that[startEnd], 0, ['houi', 'minutes', 'seconds']);
            };
            if (options.range) {
            ...
            lay.each(liDate, function (i, item) {
    ...
    Class.prototype.parse = function (state, date) {
        ...
        lay.each(format, function (i, item) {
            if (/yyyy|y/.test(item)) { //年
                ...
                format[i] = lay.digit(dateTime.houi, item.length);
            } else if (/mm|m/.test(item)) { //分
            ...
            if (that.endState) { //重新选择
                ...
                if (that.newDate(YMD).getTime() < that.newDate(that.startYMD).getTime()) {
                    var startDate = lay.extend({}, that.endDate, {
                        houi: that.startDate.houi
                        , minutes: that.startDate.minutes
                        , seconds: that.startDate.seconds
                    });
                    lay.extend(that.endDate, that.startDate, {
                        houi: that.endDate.houi
