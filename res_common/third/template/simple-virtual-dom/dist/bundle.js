  ...
  _.each(moves, function (move) {
    ...
    if (move.type === 0) { // remove item
      if (staticNodeList[index] === node.childNodes[index]) { // maybe have been removed for inserting
        node.childNodes[index]&&node.removeChild(node.childNodes[index]);
      }
    ...
    default:
      ...
      if(key =='selected'){
        ...
        if( parentNode&& parentNode.options){
          try{
            for(var i= 0,len= parentNode.options.length;i<len;i++){
              if(parentNode.options[i] == node){
                node.parentNode.selectedIndex = i;
                break;
              }
            }
          }catch(err){}
