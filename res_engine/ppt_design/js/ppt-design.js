!(function (win, $) {
    ...
    var AbsoluteDesign = {
        ...
        "class": "AbsoluteDesign",
        pageCls: 'page-pabs',
        pageDomAttrPrefix: 'page-pabs-',
        controlDomAttrPrefix: 'control-pabs-',
        ...
        //需要平铺的样式属性，font-size不在这处理
        styleStringKeys:('' +
        'backgroundImage,backgroundColor,backgroundRepeat,backgroundSize,display,' +
        'verticalAlign,' +
        'borderLeftColor,borderTopColor,borderBottomColor,borderRightColor,' +
        'borderLeftStyle,borderTopStyle,borderBottomStyle,borderRightStyle,' +
        'color,fontFamily,fontWeight,transform,textAlign,whiteSpace').split(','),
        ...
        findControlUrl:function(control){
            //设计态和运行态 此处不同
            if(control && control.otherAttrs.useRelation){ //存在关联关系
        ...
        trimTpl:function(tplId,data){
            if(!this.trimTplMap[tplId]){
                this.trimTplMap[tplId] = template.compile($.trim(document.getElementById(tplId).innerHTML));
            }
        ...
        ***/
        init: function () {
            ...
            $('.design-container').addClass('ab-design-clientType-'+me.clientType4Design);

            me.PageDesignTypeEnum = PageDesignTypeEnum;
            me.ControlTypeEnum = ControlTypeEnum;
            oui.loadApiConfig(
            oui.getContextPath()+"res_engine/ppt_design/pc/js/api-config.json",
            "oui.biz",function(){
                //根据url参数初始化 输入参数
                me.initByUrlParams(function(){
                    var param = oui.getParam();
                    //TOOD 联调时 修改改接口路径
                    me.loadPageDesignUrl = oui.pptAjaxConfig.loadPageByPageIdUrl;

                    //加载页面列表后，加载指定页面
                    me.loadPagesList(function(){
                        //如果是后台ajax则要用post请求
                        //这里暂时用 getData 请求

                        var pageId = me.pageId;
                        var loadPageDesignUrl = param.loadPageDesignUrl || me.loadPageDesignUrl ;
                        if(!pageId){
                            me.data =   {};
                            if(typeof me.data.controls=='string'){
                                me.data.controls = oui.parseJson(me.data.controls);
                            }
                            if(typeof me.data.style=='string'){
                                me.data.style = oui.parseJson(me.data.style);
                            }
                            if(typeof me.data.innerStyle=='string'){
                                me.data.innerStyle = oui.parseJson(me.data.innerStyle);
                            }
                            if(typeof me.data.otherAttrs=='string'){
                                me.data.otherAttrs = oui.parseJson(me.data.otherAttrs);
                            }
                            if(typeof me.data.events=='string'){
                                me.data.events = oui.parseJson(me.data.events);
                            }
                            me.transPage4Front(me.data);

                            me.init4parse();
                        }else {
                            oui.ajaxPost(loadPageDesignUrl, {
                                "PageId": pageId
                            }, function (res) {
                                ...
                                me.data = oui.parseJson(res.PageData||'{}');
                                if (typeof me.data.controls == 'string') {
                                    me.data.controls = oui.parseJson(me.data.controls);
                                }
                                if (typeof me.data.style == 'string') {
                                ...
                                me.data.style.width = me.width||me.data.style.width;
                                me.data.style.height = me.height || me.data.style.height;

                                //处理默认 宽高
                                if(me.data.style.width>1080){
                                    var w = 1080;
                                    var h = (1080*me.data.style.height/me.data.style.width).toFixed(2);
                                    me.data.style.width = w;
                                    me.data.style.height = h;
                                }
                                if(me.data.style.height>750){
                                    var h = 750;
                                    var w = (750*me.data.style.width/me.data.style.height).toFixed(2);
                                    me.data.style.width = w;
                                    me.data.style.height = h;

                                }
                                me.data.style.cellType ='px';
                                me.transPage4Front(me.data);
                                me.init4parse();
                            }, function (res) {
                            }, '加载中...');
         ...
         * @param callback
         */
        loadPagesList:function(callback){
            var me = this;
            // ajax 获取 数据
            var url = oui.pptAjaxConfig.findPagesByPptIdUrl; //findPagesByPptId
            var id = oui.getParam('pptId'); //节目id
            var pageId = oui.getParam('pageId') || ""; //页面id
            oui.ajaxPost(url,{
                "TranslateId":id
            },function(res){
                debugger
                ...
                var width = res.Width;//节目分辨率宽度
                var height = res.Height;//节目分辨率高度
                me.sourceWidth = width;//原始宽度
                me.sourceHeight = height;//原始高度

                me.width = width;
                me.height = height;
                var w = oui.getParam('width');
                var h = oui.getParam('height');
                if(w&&h){
                    w = parseFloat(w);
                    h = parseFloat(h);
                    me.width = w;
                ...
                me.pptName = res.TranslateName; //节目名称
                me.pptId = id; //节目id
                me.pageId = pageId; //当前页面id
                me.pageList = data; //页面列表
                callback&&callback();
        ...
        findFieldStyle4Dom:function(item,controlInTd){
            var me = this;
            var str = me.findStyleString(item.innerStyle&&item.innerStyle.styleField,'control-style-tpl',controlInTd);
        ...
        findStyleFieldOuter4Dom:function(item,controlInTd){
            ...
            if(item.innerStyle&&item.innerStyle.styleFieldOuter){
                str = me.findStyleString(item.innerStyle&&item.innerStyle.styleFieldOuter,'control-style-tpl',controlInTd);

                if(item.innerStyle&&item.innerStyle.styleInnerOuter&&item.innerStyle.styleInnerOuter.verticalAlign){
                    str+';vertical-align:'+item.innerStyle.styleInnerOuter.verticalAlign+';';
                }
                if(item.innerStyle&&item.innerStyle.styleFieldOuter&&item.innerStyle.styleFieldOuter.textAlign){
                    str+';text-align:'+item.innerStyle.styleFieldOuter.textAlign+';';
                }
        ...
        findControlStyle4Dom:function(control,controlInTd){
            ...
            var str ='';
            if(control.style){
                str =me.findStyleString(control.style,'control-style-tpl',controlInTd);
        ...
        findControlStyle4DomDesign:function(control,controlInTd){
            ...
            if(control.style){
                str =me.findStyleString(control.style,'control-style-tpl',controlInTd);
                control.styleString = str;
                str = me.findStyleString(control.style,'control-style-tpl',controlInTd,true);
            }
            return str;
        ...
        findControlStyleBorder4Dom:function(control,controlInTd){
            ...
            var str ='';
            if(control.style){
                str =me.findStyleString(control.style,'border-style-tpl',controlInTd);
        ...
        findControlStyleInnerOuter4Dom:function(control,controlInTd){
            ...
            if(control.innerStyle&&control.innerStyle.styleInnerOuter){
                str = this.findStyleString(control.innerStyle.styleInnerOuter,'control-style-tpl',controlInTd);
        ...
        findControlInnerStyleTitle4Dom:function(control,controlInTd){
            ...
            if(control.innerStyle&&control.innerStyle.styleTitle){
                str = this.findStyleString(control.innerStyle&&control.innerStyle.styleTitle,'control-style-tpl',controlInTd);
        ...
        initByUrlParams:function(callback){
            ...
            if(param.ouiInWindowDialog && ((param.ouiInWindowDialog+'') == 'true') && (!viewType)){//openWindow
                var windowId = param.windowId;
                if((!window.opener)||(!window.opener._openMap) ||(!window.opener._openMap[windowId])){
                    //父窗体不存在，则关闭当前窗体
                    oui.getTop().oui.alert('调用页面设计器入口页面已经刷新或者关闭，请尝试关闭当前页面后，重新打开');
                ...
                paramCfg.viewType = 'openWindow';
            }else if(param.ouiInDialog && ((param.ouiInDialog+'') =='true')&&(!viewType)){//urlDialog
                var dialog = oui.getCurrUrlDialog();
            ...
            var pageDesignType = page.pageDesignType || paramCfg.pageDesignType ||'printForm';

            var designTplUrl = oui.getContextPath()+'res_engine/ppt_design/common/ppt-design-tpl.html';
            if(pageDesignType =='printForm'){
                //纯div占位 模板
                designTplUrl = oui.getContextPath()+'res_engine/ppt_design/common/ppt-design-tpl-space.html';
            }
            designTplUrl = page.designTplUrl || paramCfg.designTplUrl || designTplUrl;
            if(paramCfg.scriptPkg){//设置自定义命名空间
                var plugin = {
            ...
            var bizJsArr = [oui.getContextPath()+'res_engine/ppt_design/common/js/ppt-design.js'];//默认公共
            ...
            oui.require(bizJsArr,function(){
                ...
                oui.getTop().oui.alert('加载插件资源 bizJs 中的资源失败,请联系管理员检查');
            },false);
        },
        ...
        init4static: function () {
             ...
             * ****/
            var strKeys = 'id,parentId,bizId,detailId,name,description,configTemplate,htmlType,controlType,icon,fieldType';
        ...
        checkData: function (notTips) {
            ...
            /** 校验当前控件****/
            var messages = [];
            if (me.data.currentControl && me.data.controls.indexOf(me.data.currentControl) > -1) {
                //遇到第一个错误则退出
            ...
            if (messages && messages.length) {
                if(!notTips){
                    oui.getTop().oui.showAutoTips({content: messages[0].msg, boxStyle: 'background-color:#e07365'});
                }
                return false;
            }
            me.data.tempControl = {};
        ...
        checkControl: function (control, oneErrorBreak,noTips) {
            var me = this;
            /** 校验控件临时数据****/
            var messages = me.Validator.check(me.data.tempControl, oneErrorBreak, 'tempControl');
            if (messages && messages.length) {
        ...
        formField: function (control) { //判断控件是否是页面字段
            var flag = true;
        ...
        findStateTypeName: function () {
            ...
            if ('init,down2selectPage'.split(',').indexOf(state) > -1) {
                ...
                if (me.data.currentControl && (me.data.currentControl.htmlType == ControlTypeEnum.selectArea.name)) {
                    titleName = '选择区域属性';
                } else {
                    titleName = '控件属性';
                }
        ...
        findControlAttrsRepeat4Dom: function (control) {
            var me = this;
            return me.findObjectAttrsRepeat4Dom(me.controlProps, control, me.controlDomAttrPrefix);
        },
        ...
        findStyleString: function (style, tplId,controlInTd,rect4px) {
            ...
            if (!me.tpls[tplId]) {
                me.tpls[tplId] = template.compile(document.getElementById(tplId).innerHTML);
            }
            var bgImg = style.backgroundImage;
            if(style.backgroundImage){
                style.backgroundImage="";
            }
            var html = me.tpls[tplId]({data: me.data, style: style,controlInTd:controlInTd,rect4px:rect4px});
            html = html.substring(html.indexOf('{')+1, html.lastIndexOf('}'));
            html = html.replace(/data\:image\/jpeg\;/ig,'_imagedata_');
            var arr = html.split(';');
            ...
            html = strArr.join(';');
            html = html.replace(/_imagedata_/ig,'data:image/jpeg;');
            return html;
        },
        ...
        findDefaultControlByBizControlType: function (controlType,htmlType,obj) {
            ...
            if (controlType) {
                ...
                curr = oui.findOneFromArrayBy(me.defaultControls4Normal, function (item) {
        ...
        initBefore: function () {
            ...
            var pageDesignType = page.pageDesignType ||params.pageDesignType||'printForm';
            var typeEnum = PageDesignTypeEnum.findEnum(pageDesignType);
            var typeGlobalStyle= {};
            if(typeEnum&&typeEnum.PageGlobalStyles){
                typeGlobalStyle = oui.parseJson(oui.parseString(typeEnum.PageGlobalStyles));
            }
            ...
            me.data = $.extend(true, { //默认值设置
                ...
                tempControl: {},
                tempData: {}
            }, globalStyle,typeGlobalStyle, me.data, page);//更新时,从page回填到前端页面
            /** 转换页面业务参数相关 到样式属性中转*****/
            ...
            if (me.useControls) { //通过已有的控件列表 在绝对布局设计器中设计

                var controls = params.controls ||[];
                var subControlsMap = {};
                var mainCtrls = [];
                for(var i = 0,len=controls.length;i<len;i++){
                    if(controls[i].detailId){ //明细表中控件
        ...
        updateTableLineMaxRect:function(control){
            ...
            if(control.style.width+control.style.left>me.tableLineMaxRect.right){
                me.tableLineMaxRect.right = control.style.width+control.style.left;
            }
            if(control.style.height+control.style.top>me.tableLineMaxRect.bottom){
        ...
        inTableLineMaxRect:function(posOrRect){
            ...
            if( left < me.tableLineMaxRect.left  || right > me.tableLineMaxRect.right){
                return false;
            }
            if(top < me.tableLineMaxRect.top || bottom >me.tableLineMaxRect.bottom){
                return false;
            }
            return true;
        },
        ...
        findControlByPosOrRect:function(posOrRect,currControl){
            ...
            for(var i = 0,len=controls.length;i<len;i++){
                ...
                if(currControl && (currControl.id != curr.id)){
                    if(me.posInControl(curr,posOrRect,true)){ //只判断垂直方向是否在同一个位置
                        return curr;
                    }else{
                        //console.log('当前位置');
                        //console.log(posOrRect);
                        var top = posOrRect.top;
                        var currDistance = Math.min(Math.abs(top-curr.style.top),Math.abs(top-curr.style.top-curr.style.height));
         ...
         * @param rowIndex
         */
        removeRectInTableLineByColumnAndRow:function(tableLine,columnIndex,rowIndex){
            var me = this;
            if(!tableLine){
                return ;
            }
        ...
        findRectInTableLineByColumnAndRow:function(tableLine,columnIndex,rowIndex){
            var cellsMap = tableLine.style.cellsMap ||{};
        ...
        findMergeCellByRect:function(tableLine,rect){
            ...
            for(var i in mergeCellsMap){
                ...
                if(startColumnIndex>=mergeCell.startColumnIndex&& endColumnIndex <=mergeCell.endColumnIndex){
                    if(startRowIndex>=mergeCell.startRowIndex && endRowIndex<=mergeCell.endRowIndex){
                        return mergeCell;
                    }
                }
            }
        ...
        findRectInTableLine:function(tableLine,posOrRect,notCheckControl){
            ...
            for(var i =0,len=columnLines.length;i<len-1;i++){
                currLeft = columnLines[i].config.lineHeight+columnLines[i].fromPos.left+borderLeftWidth;
                currRight = columnLines[i+1].fromPos.left+borderLeftWidth;
                if(left >=columnLines[i].fromPos.left && (right <=currRight+columnLines[i+1].config.lineHeight)){
                    columnIndex = i;
                    hasFindLeft = true;
                    break;
                }
            ...
            for(var i =0,len=rowLines.length;i<len-1;i++){
                currTop = rowLines[i].config.lineHeight+rowLines[i].fromPos.top+borderTopWidth;
                currBottom = rowLines[i+1].fromPos.top+borderTopWidth;
                if(top >=rowLines[i].fromPos.top && (bottom<=currBottom+rowLines[i+1].config.lineHeight)){
                    rowIndex = i;
                    hasFindTop = true;
                    break;
                }
            ...
            if(mergeCell){
                ...
                rect.colspan = mergeCell.endColumnIndex -mergeCell.startColumnIndex+1;
            }
            //找出当前格子所在的合并单元格，如果存在合并单元格中,则使用合并单元格的位置

            /** 默认需要检测 是否存在控件， 在拖拽表格单元格选择区域时，则不需要检测是否存在控件*****/
            if(!notCheckControl){
                var rectInTable = me.findRectInTableLineByColumnAndRow(tableLine,rect.columnIndex,rect.rowIndex);
                if(rectInTable&&rectInTable.controlId){
                    var currField = me.data.currentControl ;
                    //当前拖拽中的控件，和表格中的单元格位置是同一个控件 则可以放置，否则不能在当前单元格放置控件了

                    if(currField  && (currField.id !=rectInTable.controlId)){//当前拖拽中的控件
        ...
        findMaxArea:function(tableLine,startRect,endRect){
            var me = this;
            var result = null;
            var startColumnIndex = startRect.columnIndex <endRect.columnIndex?startRect.columnIndex:endRect.columnIndex;
            var endColumnIndex1 =startRect.columnIndex+((startRect.colspan&&startRect.colspan>1)?(startRect.colspan-1):0);
            var endColumnIndex2 =endRect.columnIndex+((endRect.colspan&&endRect.colspan>1)?(endRect.colspan-1):0);
            var endColumnIndex = endColumnIndex1>endColumnIndex2?endColumnIndex1:endColumnIndex2;
            var startRowIndex = startRect.rowIndex<endRect.rowIndex?startRect.rowIndex:endRect.rowIndex;
            var endRowIndex1=startRect.rowIndex+((startRect.rowspan&&startRect.rowspan>1)?(startRect.rowspan-1):0);
            var endRowIndex2=endRect.rowIndex+((endRect.rowspan&&endRect.rowspan>1)?(endRect.rowspan-1):0);
            var endRowIndex =  endRowIndex1>endRowIndex2?endRowIndex1:endRowIndex2;
            ...
            if(mergeCellsMap){
                ...
                do {
                    hasChange = false;
                    var mergeCells = me.findMergeCellsByCellIndex4Inner(tableLine,startColumnIndex,startRowIndex,endColumnIndex,endRowIndex);
        ...
        findRectBySelectRect:function(currField,selectRect){
            ...
            var startRect = {
                ...
                left:currField.style.left+currField.style.borderLeftWidth+columnLines[newSelectRect.startColumnIndex].config.lineHeight+columnLines[newSelectRect.startColumnIndex].fromPos.left,
                right:currField.style.left+currField.style.borderLeftWidth+columnLines[newSelectRect.startColumnIndex+1].fromPos.left,
                top:currField.style.top+currField.style.borderTopWidth+rowLines[newSelectRect.startRowIndex].config.lineHeight+rowLines[newSelectRect.startRowIndex].fromPos.top,
                bottom:currField.style.top+currField.style.borderTopWidth+rowLines[newSelectRect.startRowIndex+1].fromPos.top
            };
            var endRect = {
                columnIndex:newSelectRect.endColumnIndex,
                rowIndex:newSelectRect.endRowIndex,
                left:currField.style.left+currField.style.borderLeftWidth+columnLines[newSelectRect.endColumnIndex].config.lineHeight+columnLines[newSelectRect.endColumnIndex].fromPos.left,
                right:currField.style.left+currField.style.borderLeftWidth+columnLines[newSelectRect.endColumnIndex+1].fromPos.left,
                top:currField.style.top+currField.style.borderTopWidth+rowLines[newSelectRect.endRowIndex].config.lineHeight+rowLines[newSelectRect.endRowIndex].fromPos.top,
                bottom:currField.style.top+currField.style.borderTopWidth+rowLines[newSelectRect.endRowIndex+1].fromPos.top
            };
            ...
            if(newSelectRect.startSelectColumnIndex>-1 || newSelectRect.startSelectRowIndex>-1){
        ...
        showOperationSelectByTableLineSelectRect:function(tableLine,rect){
            ...
            if(rect){
                $operation.find('.operation-select-area').css({
                    ...
                    background:'red',
                    border:0,
                    left:(rect.left-tableLine.style.left+tableLine.style.lineHeight )+'px',
                    top:(rect.top-tableLine.style.top+tableLine.style.lineHeight )+'px',
                ...
                var columnSelector = [];
                var rowSelector =[];
                for(var columnIndex =rect.startColumnIndex;columnIndex<=rect.endColumnIndex;columnIndex++){
                    columnSelector.push('.control-column-num-pabs[line-column-index='+columnIndex+']');
                }
                for(var rowIndex=rect.startRowIndex;rowIndex<=rect.endRowIndex;rowIndex++){
                    rowSelector.push('.control-row-num-pabs[line-row-index='+rowIndex+']');
                }
        ...
        findRectBySelectArea:function(currField,dragData){
            ...
            if(currField.htmlType =='tableLine'){
                if(dragData.startSelectColumnIndex>-1){
                    //列选择
                    var startColumnRect = me.findColumnRectByPosOrRect(currField,{left:rect.left,top:rect.top});
                    var endColumnRect = me.findColumnRectByPosOrRect(currField,{left:rect.right,top:rect.bottom});
                    var startRect = {
                        rowIndex:0,
                    ...
                    //行选择
                    var startRowRect = me.findRowRectByPosOrRect(currField,{left:rect.left,top:rect.top});
                    var endRowRect = me.findRowRectByPosOrRect(currField,{left:rect.right,top:rect.bottom});
                    var startRect = {
                        columnIndex:0,
                    ...
                    //单元格选择
                    var startRect = me.findRectInTableLine(currField,{left:rect.left,top:rect.top},true);
                    var endRect = me.findRectInTableLine(currField,{left:rect.right,top:rect.bottom},true);
        ...
        findColumnRectByPosOrRect:function(tableLine,posOrRect){
            ...
            if(hasFindLeft){
                ...
                rect.right = currRight+tableLine.style.left;
                rect.columnIndex = columnIndex;
                rect.startTop = tableLine.style.top+rowLines[0].config.lineHeight+rowLines[0].fromPos.top+borderTopWidth;
                rect.startBottom = tableLine.style.top+rowLines[1].fromPos.top+borderTopWidth;
                rect.top = tableLine.style.top+rowLines[rowLines.length-2].config.lineHeight+rowLines[rowLines.length-2].fromPos.top+borderTopWidth;
                rect.bottom = tableLine.style.top+rowLines[rowLines.length-1].fromPos.top+borderTopWidth;
            }
            return rect;
        },
        ...
        findRowRectByPosOrRect:function(tableLine,posOrRect){
            ...
            if(hasFindTop){
                ...
                rect.startLeft = tableLine.style.left+columnLines[0].config.lineHeight+columnLines[0].fromPos.left+borderLeftWidth;
                rect.startRight = tableLine.style.left+columnLines[1].fromPos.left + borderLeftWidth;

                rect.left = tableLine.style.left+columnLines[columnLines.length-2].config.lineHeight+columnLines[columnLines.length-2].fromPos.left+borderLeftWidth;
                rect.right =tableLine.style.left+ columnLines[columnLines.length-1].fromPos.left + borderLeftWidth;
        ...
        createTableColumnLines:function(currentControl){
            ...
            var widthStep = (width-lineHeight-currentControl.style.borderLeftWidth-currentControl.style.borderRightWidth)/columnSize;
        ...
        createTableRowLines:function(currentControl){
            ...
            var heightStep = (height-lineHeight-currentControl.style.borderBottomWidth-currentControl.style.borderTopWidth)/rowSize;
        ...
        findCurrPageBizId:function(){
            var me = this;
            var subId = '';
            if(me.useControls){ //使用控件回填的场景才会根据当前控件渲染可拖拽控件区域
                if(me.data&&me.data.currentControl&&(me.data.currentControl.controlType =='detail' || me.data.currentControl.controlType =='detail')){
        ...
        findDetailName:function(subId){
            var me = this;
            var curr = oui.findOneFromArrayBy(me.ctrlAreaControls||[],function(item){
                if(item.id == subId){
                    return true;
                }else{
                    return false;
        ...
        findControlTypes: function () {
            ...
            var findControls = function () {
                ...
                if (!this.controls) {
                    ...
                    this.controls = oui.findManyFromArrayBy(targetControls, function (item) {
                            if (item.htmlType == htmlType) {
                                return true;
                            }
                        }) || [];
            ...
            var findCls = function (item) {
                ...
                if (item.id) {
                    ...
                    //在当前控件列表中
                    var controlInDesign = oui.findOneFromArrayBy(me.data.controls || [], function (curr) {
                        if (curr.id == item.id) {
                            return true;
                        }
                    });
                    if (controlInDesign) {
                        cls += ' control-pabs-disable';//已经在设计区,则不不能拖拽
                    }
                }
        ...
        findPaperTypeName: function (paperType) {
            ...
            if (curr.value) {
                if (curr.value != 'self_edit') {
                    desc = curr.value + '（' + curr.width + curr.cellType + '*' + curr.height + curr.cellType + '）';
                } else {
                    desc = '自定义';
                }
            }
        ...
        findThemeUrl:function(){
            ...
            if(me.data.style.theme){
                if(me.data.style.theme =='self_edit'){
                    ...
                    if(url){
                        if(url.indexOf('http')!=0){//本地工程相对路径
            ...
            if (v) {
                ...
                if (curr) {
                    ...
                    if (v != 'self_edit') {
                        ...
                        if ((typeof me.data.style.width == 'undefined') || (typeof me.data.style.height == 'undefined')) {
                            var defPage = oui.findOneFromArrayBy(me.paperRuleTypes, function (temp) {
                                if (temp.value == 'A4') {
                                    return true;
                                }
                            });
        ...
        onBeforeRender4Props:function(){
            ...
            if(oui.browser.ie || oui.browser.isEdge) {//ie下不用虚拟dom绑定更新
        ...
        onAfterRender4Props:function(){
            ...
            if(oui.browser.ie || oui.browser.isEdge) {//ie下不用虚拟dom绑定更新
                ...
                me.timer4AfterRender4Props = setTimeout(function(){
                    var $errorTabs= $('.fielderror-info').closest('.design-attribute-item');
                    if($errorTabs&&$errorTabs.length){
                        $errorTabs.find('.design-set-attribute-tit').addClass('attribute-tit-active');
        ...
        bindDrag: function (options) {
            ...
            var $dropContainer = $(options.dropContainer || options.scrollContainer || 'body');
            var $dragScrollContainer = $(options.dragScrollContainer || 'body'); //拖拽区域（新建）的滚动区域的容器
            var cancelSeletor = options.cancelSelector || '.control-pabs-disable';
            ...
            $doc.on('mousedown', options.dragSelector, function (e) {
                ...
                if($(e.target).is('.proxy-drag') || ($inProxyDrag&&$inProxyDrag.length)){
                    return false;
                }
                var dropPos = getElementPositionXY($dropContainer[0]);
                ...
                var isSelectColumn = $(el).hasClass('control-column-num-pabs');//选择列
                var selectColumnEl=null;
                var selectRowEl = null;
                if(!isSelectColumn){
                    if($(el).closest('.control-column-num-pabs').length>0){
                        isSelectColumn = true;
                        selectColumnEl = $(el).closest('.control-column-num-pabs')[0];
                    }
                }else{
                    selectColumnEl = el;
                }
                ...
                if(!isSelectRow){
                    var isSelectRow = $(el).hasClass('control-row-num-pabs');//选择行
                    if($(el).closest('.control-row-num-pabs').length>0){
                        isSelectRow = true;
                        selectRowEl = $(el).closest('.control-row-num-pabs')[0];
                    }
                }else{
                    selectRowEl = el;
                }
                ...
                var resizerEl =null;
                if(isSelectColumn){
                    startSelectColumnIndex =  parseInt($(selectColumnEl).attr('line-column-index'));
                }
                if(isSelectRow){
                ...
                //console.log(el);
                if (!isResizer) {
                    if (!$(el).is(options.dragSelector)) { //如果不是拖拽元素则不能进行拖拽
                        if ($(el).is(cancelSeletor)) {//不允许拖拽则返回
                            return false;
                        }
                ...
                if($(el).is(operationSelector) && isUpdate){
                    //判断是否是操作层 在拖拽
                    el = $('#'+el.getAttribute('control-pabs-id'))[0];//指定拖拽对应的目标元素
                }
                ...
                if((!isOnlyDrag4Table) &&(!isResizer)){//拖拽改变位置的状态下，并且不是拖拽按钮上则获取最外层是否是编辑状态，进行区域选择
                ...
                if(resizerEl){
                    ...
                    if(isResizerColumn){
                        if(resizeColumnIndex>=0&&resizeColumnIndex<lineColumnLength){
                            if(resizeColumnIndex>0){//第一个不处理
                                var $resizerColumnPrev = $(resizerEl).prev('.line-column-resizer');
                                if($resizerColumnPrev&&$resizerColumnPrev.length){
                                    fromResizeLeft = Number($resizerColumnPrev.css('left').replace('px','')||'0');
                                    fromResizeLeft +=$resizerColumnPrev.outerWidth();
                                }
                            }

                            if(resizeColumnIndex!=lineColumnLength-1){//最后一个不处理
                                var $resizerColumnNext = $(resizerEl).next('.line-column-resizer');
                                if($resizerColumnNext&&$resizerColumnNext.length){
                                    toResizeLeft =  Number($resizerColumnNext.css('left').replace('px','')||'0');
                ...
                if(!dragCfg){
                    me.dragCfg = {
                        e:e, el:el, isUpdate:isUpdate, isResizer:isResizer,isSelectArea:isSelectArea,
                ...
                var moveFun = function (ev) {
                    ...
                    //console.log($(ev.target).closest('.control-pabs-component')[0]&&$(ev.target).closest('.control-pabs-component')[0].id);
                    ...
                    if (dragStatus == 'start' || dragStatus == 'dragging') {
                        ...
                        if (dragStatus == 'start') {
                            ...
                            var width, height;
                            if (!isUpdate) {
                                var currField = me.getElConfigByKeys(el, me.controlProps, me.controlDomAttrPrefix);
                            ...
                            if (!isUpdate) { //新建时的拖拽 ，需要剔除新建区域 滚动条滚动位置
                            ...
                            if(resizerEl){
                                try{
                                    startResizeTop = Number($(resizerEl).css('top').replace('px','')||'0');
                                }catch(err){
                                    startResizeTop =0;
                                }

                                try{
                                    startResizeLeft = Number($(resizerEl).css('left').replace('px','')||'0');
                                }catch(err){
                                    startResizeLeft =0;
                                }
                                if(isResizerColumn || isResizerRow){
                                    $(resizerEl).addClass('line-dragging');
                                    if((resizeColumnIndex==0 || resizeColumnIndex==lineColumnLength-1)||(resizeRowIndex==0 || resizeRowIndex==lineRowLength-1)){
                                        //改变 大小
                        ...
                        dragData.mouseX4relative = currPos.x + scrollContainerLeft - dropPos.x;
                        dragData.mouseY4relative = currPos.y +scrollContainerTop -dropPos.y;
                        if (isStart) {
                            ...
                            if (isUpdate) { //判断当前事件的元素 是否在指定元素范围内
                                left -= scrollContainerLeft;
                                top -= scrollContainerTop;
                            }
                        }
                        ...
                        if ((!isResizer) && (!isSelectArea)) {
                            ...
                            if((isResizerRow || isResizerColumn)&&resizerEl){
                                ...
                                if(isResizerRow){
                                    //改变行高
                                    //拖拽改变行高的线
                                    dragData.resizeTop = top - dragData.startY+dragData.startResizeTop;
                                    ...
                                    if(dragData.resizeTop+dragData.startY>dragData.dropPos.height){
                                        dragData.resizeTop =  dragData.dropPos.height-dragData.startY-dragData.lineWidth;
                                    }
                                    ...
                                    //改变高度
                                    if((resizeRowIndex == 0) || (resizeRowIndex == dragData.lineRowLength-1)){ //改变 高度
                                        if(resizeRowIndex ==0){
                                            /** 行高调整和位置调整****/
                                            ...
                                            if(dragData.resizeHeight+dragData.startY>dropPos.height){
                                                dragData.resizeHeight = dropPos.height -dragData.startY;
                                            }
                                        }
                                    }
                                }else {
                                    //改变列宽
                                    //拖拽改变行高的线
                                    dragData.resizeLeft =left - dragData.startX+dragData.startResizeLeft;
                                    ...
                                    if(dragData.resizeLeft+dragData.startX>dragData.dropPos.width){
                                        dragData.resizeLeft =  dragData.dropPos.width-dragData.startX;
                                    }
                                    $(resizerEl).css('left',dragData.resizeLeft+'px');
                                    var resizeFix = dragData.resizeLeft-dragData.startResizeLeft;

                                    //改变宽度
                                    if((resizeColumnIndex==0) || (resizeColumnIndex == dragData.lineColumnLength-1)){
                                        if(resizeColumnIndex ==0){
                                            ...
                                            if(dragData.resizeWidth+dragData.startX >dropPos.width){
                        ...
                        if (isResizerColumn){
                            if(resizeColumnIndex==0 || resizeColumnIndex == lineColumnLength-1){
                                if(resizeColumnIndex ==0){
                ...
                var mouseup = function (ev) {
                    ...
                    //        $('#canvas_box').remove();
                    //        var html='<div id="canvas_box" style="display:none;" class="ppt-paper-area-canvas" style="width:'+$dropContainer[0].scrollWidth+'px;height:'+$dropContainer[0].scrollHeight+'px"></div>';
        ...
        removeMergeCellsByCellIndex:function(tableLine,startColumnIndex,startRowIndex,endColumnIndex,endRowIndex){
            var me = this;
            var mergeCells = me.findMergeCellsByCellIndex(tableLine,startColumnIndex,startRowIndex,endColumnIndex,endRowIndex);
            if(mergeCells&&mergeCells.length){
                for(var i= 0,len=mergeCells.length;i<len;i++){
        ...
        findMergeCellsByCellIndex4Inner:function(tableLine,startColumnIndex,startRowIndex,endColumnIndex,endRowIndex){
            var me = this;
            var arr = [];
            var mergeCellsMap  = tableLine.style.mergeCellsMap;
            if(!mergeCellsMap){
            ...
            for(var key in mergeCellsMap){
                ...
                inner:for(var currSC=startColumnIndex;currSC<=endColumnIndex;currSC++){
        ...
        /** 在指定区域内找出所有 的合并单元格*****/
        findMergeCellsByCellIndex:function(tableLine,startColumnIndex,startRowIndex,endColumnIndex,endRowIndex){
            var me = this;
            var arr = [];
            var mergeCellsMap  = tableLine.style.mergeCellsMap;
            if(!mergeCellsMap){
            ...
            for(var key in mergeCellsMap){
                ...
                if(sc>=startColumnIndex && ec<=endColumnIndex && sr>=startRowIndex && er<=endRowIndex ){
                    arr.push(mergeCellsMap[key]);
                }
            }
            return arr;
        ...
        /** 根据 开始结束位置 获取 控件列表 ****/
        findRectsInTableLineByCellIndex:function(tableLine,startColumnIndex,startRowIndex,endColumnIndex,endRowIndex){
            var me = this;
            var arr = [];
            /*** 行列遍历***/
        ...
        inDetail:function(control){
            var me = this;
            var flag = false;
            if(control.style&&control.style.rect&&control.style.rect.tableLineId){
        ...
        getControlRenderDataById:function(controlId){
            ...
            if(control){
                ...
                data.cls = 'control-field-value-pabs';
        ...
        findTableCell:function(tableLine,columnLineIndex,lineIndex){
            ...
            if(mergeCellsMap){
                for(var key in mergeCellsMap){
                    ...
                    if((cell.startColumnIndex==columnLineIndex) && (cell.startRowIndex ==lineIndex)){
                        result.columnIndex = cell.startColumnIndex;
                        result.rowIndex = cell.startRowIndex;
                        break;
                    }
                    /** 找到合并单元格，并且 被合并单元格包含，则 返回不存在*****/
                    if((columnLineIndex>=cell.startColumnIndex) && (columnLineIndex<=cell.endColumnIndex)){
                        if((lineIndex>=cell.startRowIndex) && (lineIndex<=cell.endRowIndex)){
                            result.columnIndex = cell.startColumnIndex;
                            result.rowIndex = cell.startRowIndex;
                            break;
                        }
        ...
        event2merge:function(cfg){
            ...
            if(me.selectRect.isSelectColumn || me.selectRect.isSelectRow){
                me.selectRect = me.findMaxArea(currentControl,me.selectRect.startRect,me.selectRect.endRect);
            }
            ...
            var rects = me.findRectsInTableLineByCellIndex(currentControl,startColumnIndex,startRowIndex,endColumnIndex,endRowIndex);
            if(rects&&rects.length>1){
                oui.getTop().oui.alert('当前选择单元格区域含有多个控件不能进行合并');
                return ;
            }
            //二、判断 区域内是否有 合并的单元格，需要执行删除，添加新的合并单元格信息
            me.removeMergeCellsByCellIndex(currentControl,startColumnIndex,startRowIndex,endColumnIndex,endRowIndex);
            me.addMergeCell(currentControl,oui.parseJson(oui.parseString(me.selectRect)));
            ...
            if(rects.length==1){
                ...
                newRect.rowspan =me.selectRect.endRowIndex-me.selectRect.startRowIndex+1; //跨行
                newRect.colspan = me.selectRect.endColumnIndex-me.selectRect.startColumnIndex+1;//跨列
                ...
                me.removeRectInTableLineByColumnAndRow(currentControl,rects[0].columnIndex,rects[0].rowIndex);
                ...
                //四 更新单元格中的控件
                var controlEl = $('#'+controlInCell.id,'.ppt-paper-area')[0];
        ...
        event2split:function(cfg){
            ...
            var mergeCells = me.findMergeCellsByCellIndex(currentControl,startColumnIndex,startRowIndex,endColumnIndex,endRowIndex);
            var controlIds= [];
            var controlMap = {};
            if((!mergeCells) || (!mergeCells.length)){
                return ;
            ...
            for(var i= 0,len=mergeCells.length;i<len;i++){
                var rects = me.findRectsInTableLineByCellIndex(currentControl,mergeCells[i].startColumnIndex,mergeCells[i].startRowIndex,mergeCells[i].endColumnIndex,mergeCells[i].endRowIndex);
                if(rects&&(rects.length == 1)){
                    //有且只有一个控件
                    var newRect = {
                        controlId:rects[0].controlId,
                    ...
                    newRect.right = tableLeft+borderLeftWidth+columns[mergeCells[i].startColumnIndex+1].fromPos.left;
                    newRect.bottom = tableTop+borderTopWidth+rows[mergeCells[i].startRowIndex+1].fromPos.top;
                    // 删除之前的单元格信息,并设置新的单元格信息,更新控件的单元格位置和高宽
                    var controlInCell = me.getControlById(newRect.controlId);
                    me.removeRect4Control(controlInCell);
                    me.removeRectInTableLineByColumnAndRow(currentControl,rects[0].columnIndex,rects[0].rowIndex);
            ...
            if(controlIds&&controlIds.length){
                ...
                $(selector,'.ppt-paper-area').each(function(){
        ...
        event2insertColumn4prev:function(cfg){
            ...
            /** 选择了列头 ,处理批量插入****/
            var addSize = me.selectRect.endColumnIndex-me.selectRect.startColumnIndex+1;
            //一、增加线条一根在指定位置前面
            ...
            for(var i=startColumnIndex+ addSize,len=lines.length;i<len;i++){
            ...
            for(var key in cellsMap){
                ...
                if(rect.columnIndex >= startColumnIndex){
                    ...
                    if(rect.colspan&& rect.colspan>1){//跨列单元格判断
                        if(startColumnIndex>rect.columnIndex&& (rect.columnIndex+rect.colspan-1>=startColumnIndex)){
            ...
            if(mergeCellsMap){
                ...
                for(var i in mergeCellsMap){
                    ...
                    if(rect.startColumnIndex >= startColumnIndex){
                        ...
                        if(rect.startColumnIndex<startColumnIndex && rect.endColumnIndex>=startColumnIndex){
                            rect.endColumnIndex+=addSize;
                        }
                    }
                    newMergeCellsMap[rect.startColumnIndex+'_'+rect.startRowIndex] = rect;
                }
        ...
        event2insertRow4prev:function(cfg){
            ...
            for(var key in cellsMap){
                ...
                if(rect.rowIndex >= startRowIndex){
                    ...
                    if(rect.rowspan&& rect.rowspan>1){//跨行单元格判断
                        if(startRowIndex>rect.rowIndex&& (rect.rowIndex+rect.rowspan-1>=startRowIndex)){
            ...
            if(mergeCellsMap){
                ...
                for(var i in mergeCellsMap){
                    ...
                    if(rect.startRowIndex >= startRowIndex){
                        ...
                        if(rect.startRowIndex<startRowIndex && rect.endRowIndex>=startRowIndex){
                            rect.endRowIndex+=addSize;
                        }
                    }
                    newMergeCellsMap[rect.startColumnIndex+'_'+rect.startRowIndex] = rect;
        ...
        event2removeColumn:function(cfg){
            ...
            var removeSize = me.selectRect.endColumnIndex-me.selectRect.startColumnIndex+1;
            //一、删除指定位置的线条
            var lines = currentControl.style.columnLines;
            ...
            var currLine = currentControl.style.columnLines[startColumnIndex];
            var currWidth =  currentControl.style.columnLines[endColumnIndex+1].fromPos.left-  currLine.fromPos.left;
            lines.splice(startColumnIndex,removeSize);
            //二、更新指定位置以及后面的所有线的信息
            if(currentControl.style.width-currWidth<30){
                currWidth = currentControl.style.width- currentControl.style.columnLines[1].fromPos.left+  currentControl.style.columnLines[0].fromPos.left;
            }
        ...
        event2removeRow:function(cfg){
            ...
            var removeSize = me.selectRect.endRowIndex-me.selectRect.startRowIndex+1;
            var lines = currentControl.style.rowLines;
            if(removeSize>=lines.length-2){
                removeSize= lines.length-2;
            }
            //一、删除指定位置的线条
            var currLine = currentControl.style.rowLines[startRowIndex];
            var currHeight =  currentControl.style.rowLines[endRowIndex+1].fromPos.top-  currLine.fromPos.top;
            if(currentControl.style.height -currHeight<30){
                currHeight = currentControl.style.height-currentControl.style.rowLines[1].fromPos.top+  currentControl.style.rowLines[0].fromPos.top;
            }
        ...
        buildHtmlCanvasByControlId: function (id, callback) {
            var me = this;
            var control = me.getControlById(id);
            var html = oui.getById('items').getHtmlByTplId('item-canvas-div-tpl', {item: control});
            var canvasOuterHtml = oui.getById('items').getHtmlByTplId('item-canvas-object-tpl', {item: control});
            var $paperArea = $('.ppt-paper-area');
            ...
            html2canvas($el[0], {canvas: canvas, width: canvas.width, height: canvas.height}).then(function (canvas) {
                ...
                var imgData = canvas.getContext("2d").getImageData(borderLeft, borderTop, canvas.width - borderRight, canvas.height - borderBottom);


                context.putImageData(imgData, 0, 0);
                var newImgData = context.getImageData(0, 0, newCanvasWidth, newCanvasHeight);
        ...
        canvasRunner: function (size) {
            ...
            for (var i = 0, len = size; i < len; i++) {
                queue.push({
                    ...
                    run: function () {
                        currIdx = this.idx;
                        com.oui.absolute.AbsoluteDesign.buildCanvasByControlId(com.oui.absolute.AbsoluteDesign.data.controls[0].id, function () {
        ...
        getControlIdxById:function(id){
            var me = this;
            var idx = -1;
            var currentControl = oui.findOneFromArrayBy(me.data.controls || [], function (item,index) {
                if (item.id == id) {
                    idx = index;
                    return true;
                }
        ...
        getControlById: function (id) {
            var me = this;
            /*** 先从控件中找****/
            var currentControl = oui.findOneFromArrayBy(me.data.controls || [], function (item) {
                if (item.id == id) {
                    return true;
                }
            });
        ...
        removeControlsByIds:function(ids){
            ...
            for(var i= 0,len=ids.length;i<len;i++){
                ...
                if(curr){
                    disableSelector.push('.control-pabs-disable[control-pabs-id='+curr.id+']');
                }
                me.removeControlById(ids[i]);
            }
            //$('.design-item[control-pabs-bizControlid]')

            var selector ='#'+ids.join(',#');
            $(selector,'.ppt-paper-area').remove();
            if(disableSelector&&disableSelector.length){
                $(disableSelector.join(','),'.design-left').removeClass('control-pabs-disable'); //取消禁用
            }
        },
        removeControlById: function (id) {
            ...
            /** 更新当前最上层元素***/
            if(currentControl && currentControl.style.zIndex == me.data.style.currControlZIndex){
            ...
            if(currentControl&&currentControl.htmlType =='tableLine'){
                ...
                //判断控件是否在某个虚拟表格单元格中
                if(currentControl && currentControl.style.rect && currentControl.style.rect.tableLineId){
                    var tableLineControl = me.findTableLineControl(currentControl.style.rect.tableLineId);
                    if(tableLineControl){
                        var rect = currentControl.style.rect;
                        me.removeRectInTableLineByColumnAndRow(tableLineControl,rect.columnIndex,rect.rowIndex);
                    }
                }
            }
            if (currIdx > -1) {
        ...
        bindCheckEvents: function () {
            var me = this;
            me.setInterceptFuns("down2selectPaperArea,event2cloneControl,event2preview,event2save,event2selectBoxArea,event2print,event2triggerPaperDown", function () {
                return me.checkData();
            });
        },
        ...
        bindColorChangeEvents:function(){
            ...
            $(document).on('mouseenter', '[type=spectrum]', function () {
                ...
                $(this).spectrum({
                    ...
                    palette: [
                        [
                            "rgb(208,215,229)","rgb(0, 0, 0)", "rgb(67, 67, 67)", "rgb(102, 102, 102)",
                        ...
                        [  "rgb(221, 126, 107)", "rgb(234, 153, 153)", "rgb(249, 203, 156)", "rgb(255, 229, 153)", "rgb(182, 215, 168)",
                            "rgb(162, 196, 201)", "rgb(164, 194, 244)", "rgb(159, 197, 232)", "rgb(180, 167, 214)", "rgb(76, 17, 48)"],
                        ["rgb(213, 166, 189)",
                        ...
                        ["rgb(166, 77, 121)",
                            ...
                            "rgb(19, 79, 92)", "rgb(17, 85, 204)", "rgb(11, 83, 148)", "rgb(53, 28, 117)"],
                        ["rgb(116, 27, 71)",
        ...
        updateColor: function (el, tinycolor) {
            ...
            $(el).next('.set-color-info').text(color_result||'transparent');
            var invokeProp = $(el).attr('invoke-prop');
            invokeProp&&oui.JsonPathUtil.setObjByPath(invokeProp,_self.data,color_result,true);
        ...
        resetTableLineEditStateBy:function(fun){
            ...
            if(selector&&selector.length){
                $(selector.join(',')).removeClass('drag-select-area').find('[oui-e-click=event2editTable]').attr('title','编辑完成');
            }
        },
        bindDragEvents:function(){
            ...
            var compareRect = function (curr, item) {
                ...
                if (Math.abs(currLeft - targetLeft) < 1 || Math.abs(currLeft - targetLeft - targetWidth) < 1) {
                    arr.push('left');
                }
                if (Math.abs((currLeft + currWidth) - (targetLeft + targetWidth)) < 1 || Math.abs((currLeft + currWidth) - (targetLeft)) < 1) {
                    arr.push('right');
                }

                if (Math.abs((currLeft * 2 + currWidth) / 2 - (targetLeft * 2 + targetWidth) / 2) < 1) {
                    arr.push('center');
                }
                if (Math.abs(currTop - targetTop) < 1 || Math.abs(currTop - targetTop - targetHeight) < 1) {
                    arr.push('top');
                }
                if (Math.abs((currTop + currHeight) - (targetTop + targetHeight)) < 1 || Math.abs((currTop + currHeight) - (targetTop)) < 1) {
                    arr.push('bottom');
                }

                if (Math.abs((currTop * 2 + currHeight) / 2 - (targetTop * 2 + targetHeight) / 2) < 1) {
                    arr.push('middle');
                }
                return arr;
            };
            ...
            this.bindDrag({
                ...
                cancelSelector: '.control-pabs-disable,.design-copy-ctrl,.design-del-ctrl,.tableLine-edit-icon,.proxy-drag-placeholder,.proxy-dragging-select-area',//不允许拖拽元素
                dragSelector: '.design-item,.tree-node-page,.control-pabs-component', //可拖拽元素
                ...
                dropContainer: '.ppt-paper-area',//放置元素区域的容器
                down: function (cfg) {
                    var e=cfg.e, el=cfg.el, isUpdate=cfg.isUpdate, isResizer=cfg.isResizer;
                    var $inPreview = $(el).closest('.preview-box');
                    if($inPreview&&$inPreview.length){
                        return  false;
                    }
                    /** 拖拽前校验**/
                    ...
                    if (isUpdate) {
                        if(me.currAccordingActiveId =='absoluteProps-pageProps-base' || me.currAccordingActiveId =='absoluteProps-pageProps-setttings'){
                            me.currAccordingActiveId ='';
                        }
                        //拖拽的是操作层
                        //不同层做高亮
                        $('.active','.ppt-paper-area').removeClass('active');
                        $(el).addClass('active');
                        var id = el.getAttribute('id');
                        if (id) { // 内容种的元素拖拽，选中时，显示属性面板
                            ...
                            if(currentControl.htmlType =='tableLine' && (!me.isDragging4SelectArea(currentControl))){
                                //非编辑状态 不用显示操作层
                                me.hideOperationArea();
                            }else{
                                // 获取上一次 的区域选择
                ...
                dragStart: function (cfg) {
                    var e=cfg.e, el=cfg.el, $proxy=cfg.$proxyDrag, isUpdate=cfg.isUpdate, isResizer=cfg.isResizer;
                    /*
                    ...
                    if($(el).hasClass('control-pabs-tableLine')){
                        //拖拽表格 场景 需要在代理层显示 按钮
                        $proxy.find('.tableLine-drag-icon,.tableLine-edit-icon').show();
                    }else{
                        $proxy.find('.tableLine-drag-icon,.tableLine-edit-icon').hide();
                    }
                    if ($(el).hasClass('control-pabs-component')) {
                        $proxy.addClass('control-pabs-component');
                    } else {
                        $proxy.removeClass('control-pabs-component');
                    }
                    //console.log('dragStart');
                    //console.log('curr:'+me.data.currentControl.id);
                },
                dragging: function (cfg) {
                    var e=cfg.e, el=cfg.el, $proxy=cfg.$proxyDrag, dragData=cfg.dragData, isUpdate=cfg.isUpdate, isResizer=cfg.isResizer,$proxyPlaceholder=cfg.$proxyPlaceholder;
                    ...
                    var left  =dragData.x;
                    var top = dragData.y;
                    if(cfg.isResizerColumn&&(dragData.resizeColumnIndex==0 || dragData.resizeColumnIndex == dragData.lineColumnLength-1)){
                        //宽度调整
                    ...
                    if(cfg.isResizerRow&&(dragData.resizeRowIndex==0 || dragData.resizeRowIndex == dragData.lineRowLength-1)){
                        //宽度调整
                    ...
                    if(hideLines&&hideLines.length){
                        var hideLineSelector = '.proxy-line-'+hideLines.join(',.proxy-line-');
                    ...
                    var mousePos4realtive = {left:dragData.mouseX4relative,top:dragData.mouseY4relative};

                    var isInTableLine = false;
                    $proxyPlaceholder.removeClass('proxy-drag-placeholder-normalForm');//非瀑布流占位
                    /** 表格控件 内部不能再拖拽 表格控件 , 截取 页面*****/
                    if((!isResizer)&&( currentControl.htmlType != 'tableLine' && currentControl.htmlType != 'selectArea')){
                    ...
                    }else if(cfg.isSelectArea){ //当前拖拽的目的是选择区域 则进行 高亮占位计算处理
                    ...
                    if((!cfg.isSelectArea) &&(!isInTableLine)){ //非选择区域，拖拽的目的 是为了放置控件
                        if(me.data.pageDesignType &&(me.data.pageDesignType =='normalForm') &&(!isResizer)){ //查找控件占位
                            var targetControl = me.findControlByPosOrRect(mousePos4realtive,currentControl);//在指定控件位置下方放置占位
                            if(targetControl){
                                $proxyPlaceholder.addClass('proxy-drag-placeholder-normalForm');//瀑布流占位
                                var atTop = me.isPosAtControlHaffTop(targetControl,mousePos4realtive);
                                if(atTop){ //插入到前面
                                    $proxyPlaceholder.css({
                                        display:'block',
                                        'z-index':999,
                                        left:(targetControl.style.left-dragData.scrollContainerLeft+dragData.dropPos.x)+'px',
                                        top:(targetControl.style.top-dragData.scrollContainerTop+dragData.dropPos.y)+'px',
                                    ...
                                    $proxyPlaceholder.css({
                                        ...
                                        'z-index':999,
                                        left:(targetControl.style.left-dragData.scrollContainerLeft+dragData.dropPos.x)+'px',
                                        top:(targetControl.style.top+targetControl.style.height-dragData.scrollContainerTop+dragData.dropPos.y)+'px',
                                        width:(currentControl.style.width)+'px',
                                        height:(20)+'px'
                                    });
                                }
                ...
                dragEnd: function (cfg) {
                    var e=cfg.e, el=cfg.el, $proxy=cfg.$proxyDrag, dragData=cfg.dragData, isUpdate=cfg.isUpdate, isResizer=cfg.isResizer;
                    /*
                     e:ev, el:el, isUpdate:isUpdate, isResizer:isResizer,
                     resizeRowIndex:resizeRowIndex,
                     resizeColumnIndex:resizeColumnIndex,
...
//更新图片地址
            window.postMessage({
                cmd:'cmd4updatePageImgUrl',//加载系统和系统菜单
                param:{
                    pageId:me.pageId
                }
            },'*');
            //如果没有从历史加载才执行变更
            if(!me.ready2loadHistory){
                //更新缓存对象
                window.postMessage({
                    cmd:'cmd4addHistory',//加载系统和系统菜单
                    param:{
                        pageId:me.pageId
                    }
                },'*');
        ...
        refreshTableLineMergeCell:function(tableLine,columnIndex,rowIndex){
            ...
            if(cell){
                ...
                var left = tableLeft+borderLeftWidth+columns[sc].config.lineHeight+columns[sc].fromPos.left;
                var top = tableTop+borderTopWidth+rows[sr].config.lineHeight+rows[sr].fromPos.top;
        ...
        /** 刷新合并单元格 中控件 的 位置****/
        refreshTableLineMergeCellControls:function(tableLine,callback,filter,columnIndex,rowIndex){
            var me = this;
            if(tableLine.htmlType !='tableLine'){
                return ;
            }
            ...
            if((typeof columnIndex != 'undefined') &&(typeof rowIndex != 'undefined') ){
                me.refreshTableLineMergeCellControl(tableLine,columnIndex,rowIndex,callback,filter);
            }else{
                //更新所有
                var mergeCellsMap= tableLine.style.mergeCellsMap||{};
                for(var i in mergeCellsMap){
                    ...
                    if(cell){
                        var sc = cell.startColumnIndex;
                        var sr = cell.startRowIndex;
                        me.refreshTableLineMergeCellControl(tableLine,sc,sr,callback,filter);
                    }
                }
            }
        },
        /** 刷新合并单元格中控件的位置****/
        refreshTableLineMergeCellControl:function(tableLine,columnIndex,rowIndex,callback,filter){
            var me = this;
            var mergeCellsMap= tableLine.style.mergeCellsMap||{};
            var tableLeft = tableLine.style.left;
            var tableTop = tableLine.style.top;
            ...
            if(cell){
                ...
                cell.right = right;
                cell.bottom =bottom;
                var rect = me.findRectInTableLineByColumnAndRow(tableLine,columnIndex,rowIndex);
                if(rect&&rect.controlId&&rect.tableLineId){
                    var isChange= false;
                    var control = me.getControlById(rect.controlId);
                    if(rect.left !=left){
        ...
        updateTableLines:function(currField,cfg){
            var me = this;
            var e=cfg.e, el=cfg.el, $proxy=cfg.$proxyDrag, dragData=cfg.dragData, isUpdate=cfg.isUpdate, isResizer=cfg.isResizer,isResizerRow=cfg.isResizerRow,isResizerColumn=cfg.isResizerColumn;
            /** 对于 控件宽度调整的处理*****/
            if(cfg.isResizerColumn&&(dragData.resizeColumnIndex==0 || dragData.resizeColumnIndex == dragData.lineColumnLength-1)){
            ...
            if(isResizerColumn){
                ...
                var resizeFix = dragData.resizeLeft-dragData.startResizeLeft;
                var lastLeft = currField.style.columnLines[resizeColumnIndex].fromPos.left;
                currField.style.columnLines[resizeColumnIndex].fromPos.left = lastLeft+resizeFix;

                //改变宽度
                if((resizeColumnIndex==0) || (resizeColumnIndex == dragData.lineColumnLength-1)){
                    if(resizeColumnIndex ==0){
                        /** 列宽调整****/
                        var currLeft = currField.style.columnLines[resizeColumnIndex].fromPos.left;
                        ...
                        if(dragData.startX + resizeFix<0){
                            resizeFix =  -dragData.startX;
                            currField.style.columnLines[resizeColumnIndex].fromPos.left = lastLeft+resizeFix;
                            currLeft=currField.style.columnLines[resizeColumnIndex].fromPos.left;
                        ...
                        var rectNextWidth = columnLines[resizeColumnIndex+1].fromPos.left-columnLines[resizeColumnIndex].fromPos.left-columnLines[resizeColumnIndex].config.lineHeight;
                        if(rectNextWidth<34){
                            rectNextWidth =34;
                            var newLeft =columnLines[resizeColumnIndex+1].fromPos.left-columnLines[resizeColumnIndex].config.lineHeight-rectNextWidth;
                        ...
                        currField.style.width  = dragData.startWidth +resizeFix;
                        var rectCurrWidth =currField.style.columnLines[resizeColumnIndex].fromPos.left-currField.style.columnLines[resizeColumnIndex-1].fromPos.left- currField.style.columnLines[resizeColumnIndex-1].config.lineHeight;
                        if(rectCurrWidth <34){
                            rectCurrWidth = 34;
                            var newLeft = rectCurrWidth+currField.style.columnLines[resizeColumnIndex-1].fromPos.left+ currField.style.columnLines[resizeColumnIndex-1].config.lineHeight;
                            var fix = newLeft -currField.style.columnLines[resizeColumnIndex].fromPos.left;
                            currField.style.width = currField.style.width+fix;
                            currField.style.columnLines[resizeColumnIndex].fromPos.left = newLeft;
                        }
                    }
                }else{
                    ...
                    if(rectCurrWidth <34){
                        rectCurrWidth = 34;
                        currField.style.columnLines[resizeColumnIndex].fromPos.left =currField.style.columnLines[resizeColumnIndex-1].fromPos.left+ currField.style.columnLines[resizeColumnIndex-1].config.lineHeight +rectCurrWidth;
                    }
                    resizeFix =  currField.style.columnLines[resizeColumnIndex].fromPos.left-lastLeft;
                    currField.style.width+=resizeFix;
                    for(var i=resizeColumnIndex+1;i<dragData.lineColumnLength;i++){
                        currField.style.columnLines[i].fromPos.left+=resizeFix;
                        if(currField.style.columnLines[i].toPos){
                            currField.style.columnLines[i].toPos.left =currField.style.columnLines[i].fromPos.left;
                        }
                    }
                }
                ...
                currField.style.rowLines[resizeRowIndex].fromPos.top = lastTop+resizeFix;
                //改变高度
                if((resizeRowIndex == 0) || (resizeRowIndex == dragData.lineRowLength-1)){ //改变 高度
                    if(resizeRowIndex ==0){
                        /** 行高调整和位置调整****/
                        ...
                        var rectNextHeight = rowLines[resizeRowIndex+1].fromPos.top-rowLines[resizeRowIndex].fromPos.top-rowLines[resizeRowIndex].config.lineHeight;
                        if(rectNextHeight<34){
                            rectNextHeight =34;
                            var newTop =rowLines[resizeRowIndex+1].fromPos.top-rowLines[resizeRowIndex].config.lineHeight-rectNextHeight;
                        ...
                        var rectCurrHeight =currField.style.rowLines[resizeRowIndex].fromPos.top-currField.style.rowLines[resizeRowIndex-1].fromPos.top- currField.style.rowLines[resizeRowIndex-1].config.lineHeight;
                        if(rectCurrHeight <34){
                            rectCurrHeight = 34;
                            var newTop = rectCurrHeight+currField.style.rowLines[resizeRowIndex-1].fromPos.top+ currField.style.rowLines[resizeRowIndex-1].config.lineHeight;
                            var fix = newTop -currField.style.rowLines[resizeRowIndex].fromPos.top;
                            currField.style.height = currField.style.height+fix;
                    ...
                    if(rectCurrHeight <34){
                        rectCurrHeight = 34;
                        currField.style.rowLines[resizeRowIndex].fromPos.top =currField.style.rowLines[resizeRowIndex-1].fromPos.top+ currField.style.rowLines[resizeRowIndex-1].config.lineHeight +rectCurrHeight;
                    }
                    resizeFix = currField.style.rowLines[resizeRowIndex].fromPos.top -lastTop;
                    ...
                    for(var i=resizeRowIndex+1;i<dragData.lineRowLength;i++){
                        ...
                        if(currField.style.rowLines[i].toPos){
                            currField.style.rowLines[i].toPos.top =currField.style.rowLines[i].fromPos.top;
                        }
                    }
                }
            }
        ...
        updateControlRect4Border:function(currentControl){
            var me = this;
            me.updateControlRect(currentControl,{dragData:{
        ...
        updateTableLineControl4Border:function(currField,dragData){
            ...
            //获取倒数第二根线条位置+ 线条宽度
            var lastMinLeft = currField.style.columnLines[currField.style.columnLines.length-2].fromPos.left+ currField.style.columnLines[currField.style.columnLines.length-2].config.lineHeight;
            var currLeftValue = currField.style.columnLines[currField.style.columnLines.length-1].fromPos.left+fixWidth ;
            if(currLeftValue<lastMinLeft){
                currLeftValue = lastMinLeft;
            }
            currField.style.columnLines[currField.style.columnLines.length-1].fromPos.left = currLeftValue;
            currField.style.width = parseInt(currLeftValue+currField.style.columnLines[currField.style.columnLines.length-1].config.lineHeight+currField.style.borderLeftWidth +currField.style.borderRightWidth) ;


            //获取倒数第二根线条位置+ 线条宽度
            var lastMinTop = currField.style.rowLines[currField.style.rowLines.length-2].fromPos.top+ currField.style.rowLines[currField.style.rowLines.length-2].config.lineHeight;
            var currTopValue = currField.style.rowLines[currField.style.rowLines.length-1].fromPos.top+fixHeight;
            if(currTopValue<lastMinTop){
                currTopValue = lastMinTop;
            }
            currField.style.rowLines[currField.style.rowLines.length-1].fromPos.top = currTopValue;
            currField.style.height = parseInt(currTopValue+currField.style.rowLines[currField.style.rowLines.length-1].config.lineHeight+currField.style.borderTopWidth +currField.style.borderBottomWidth)  ;

            // 单元格宽度不能小于34
            var rectCurrWidth =currField.style.columnLines[currField.style.columnLines.length-1].fromPos.left-currField.style.columnLines[currField.style.columnLines.length-2].fromPos.left- currField.style.columnLines[currField.style.columnLines.length-2].config.lineHeight;
            if(rectCurrWidth <34){
                rectCurrWidth = 34;
                var newLeft = rectCurrWidth+currField.style.columnLines[currField.style.columnLines.length-2].fromPos.left+ currField.style.columnLines[currField.style.columnLines.length-2].config.lineHeight;
                var fix = newLeft -currField.style.columnLines[currField.style.columnLines.length-1].fromPos.left;
                currField.style.width = parseInt(currField.style.width+fix);
                currField.style.columnLines[currField.style.columnLines.length-1].fromPos.left = newLeft;
            }

            // 单元格高度不能小于34
            var rectCurrHeight =currField.style.rowLines[currField.style.rowLines.length-1].fromPos.top-currField.style.rowLines[currField.style.rowLines.length-2].fromPos.top- currField.style.rowLines[currField.style.rowLines.length-2].config.lineHeight;
            if(rectCurrHeight <34){
                rectCurrHeight = 34;
                var newTop = rectCurrHeight+currField.style.rowLines[currField.style.rowLines.length-2].fromPos.top+ currField.style.rowLines[currField.style.rowLines.length-2].config.lineHeight;
                var fix = newTop -currField.style.rowLines[currField.style.rowLines.length-1].fromPos.top;
                currField.style.height = parseInt(currField.style.height+fix);
                currField.style.rowLines[currField.style.rowLines.length-1].fromPos.top = newTop;
            }
        },
        ...
        updateControlsInTableLine:function(tableLine,cfg){
            ...
            if(resizeColumnIndex>-1 ){
                ...
                if(resizeColumnIndex ==0 || (resizeColumnIndex == columnLength-1)){
                    eachRules.push({
                        startColumnIndex:resizeColumnIndex == columnLength-1?resizeColumnIndex-1:resizeColumnIndex,
                        startRowIndex:0,
                        endColumnIndex:resizeColumnIndex == columnLength-1?resizeColumnIndex-1:resizeColumnIndex,
            ...
            if(resizeRowIndex>-1){
                ...
                if(resizeRowIndex ==0 || (resizeRowIndex == rowLength-1)){
                    eachRules.push({
                        startColumnIndex:0,
                        startRowIndex:resizeRowIndex ==rowLength-1 ? resizeRowIndex-1:resizeRowIndex,
                        endColumnIndex:columnLength-2,
                        endRowIndex:resizeRowIndex == rowLength-1?resizeRowIndex-1:resizeRowIndex
                    });
                }else{//影响当前行和后续所有行 调整
                    eachRules.push({
                        startColumnIndex:0,
        ...
        updateControlRect:function(currField,cfg){
            var me = this;
            var e=cfg.e, el=cfg.el, $proxy=cfg.$proxyDrag, dragData=cfg.dragData, isUpdate=cfg.isUpdate, isResizer=cfg.isResizer,isResizerRow=cfg.isResizerRow,isResizerColumn=cfg.isResizerColumn;
            if(isResizerColumn || isResizerRow){//改变 表格单元格的宽度 或者高度调整
                //更新表格线条位置
                me.updateTableLines(currField,cfg);
                //改变单元格线条位置后，批量变更 控件位置和宽度
                me.updateControlsInTableLine(currField,cfg);
            }else if(currField.htmlType=='tableLine'){ //表格控件的 高度或者宽度调整特殊处理
                ...
                if (currField.htmlType == ControlTypeEnum.selectArea.name) {
                    var $paper = $('.ppt-paper-area');
                    var width4mm = me.data.style.width / $paper.width() * currField.style.width;
                    var height4mm = me.data.style.height / $paper.height() * currField.style.height;
        ...
        clearControlAndTableLineRect:function(currField){
            ...
            if(lastRect && lastRect.controlId && lastRect.tableLineId){
                ...
                me.removeRectInTableLineByColumnAndRow(lastTable,lastRect.columnIndex,lastRect.rowIndex);
        ...
        dragControl2TableLine:function(currField,cfg){
            ...
            if( currField.htmlType == 'tableLine' || currField.htmlType =='selectArea'){
                return isDrag2Table;
            }
            var mousePos4realtive = {left:dragData.mouseX4relative,top:dragData.mouseY4relative};
            ...
            if(rect.tableLineId && rect.columnIndex>-1 && rect.rowIndex>-1){
                ...
                var lastCellInTable = me.findRectInTableLineByColumnAndRow(tableLineControl,rect.columnIndex,rect.rowIndex);
                /** 判断表格单元格中 是否已经存在控件****/
                if(lastCellInTable &&lastCellInTable.controlId ){
                    ...
                    var lastRect = oui.parseJson(oui.parseString(currField.style.rect || {}));
                    //剔除控件在上一个表格单元格中的位置
                    if(lastRect.tableLineId && lastRect.controlId){
                        var lastTableLine = me.getTableLineControl(lastRect.tableLineId);
                        me.removeRectInTableLineByColumnAndRow(lastTableLine,lastRect.columnIndex,lastRect.rowIndex);
                    }
        ...
        updateAllRectsInTableLine:function(tableLineControl){
            ...
            if(tableLineControl.htmlType =='tableLine'){
                ...
                for(var i in cellsMap){
                    var rect = cellsMap[i];
                    if(rect&&rect.controlId&&rect.tableLineId){
                        var currLeft =columnLines[rect.columnIndex].config.lineHeight+columnLines[rect.columnIndex].fromPos.left+tableLeft+borderLeftWidth;
                        var currTop = rowLines[rect.rowIndex].config.lineHeight+rowLines[rect.rowIndex].fromPos.top+tableTop+borderTopWidth;
                        var currRight = columnLines[rect.columnIndex+1].fromPos.left+tableLeft+borderLeftWidth; //默认取后一列
                        var currBottom = rowLines[rect.rowIndex+1].fromPos.top+tableTop+borderTopWidth;//默认取后一行
                        /** 跨列 则获取跨列对应的位置***/
                        if(rect.colspan&&rect.colspan>1){
                            currRight = columnLines[rect.columnIndex+rect.colspan].fromPos.left+tableLeft+borderLeftWidth; //获取列位置处理
                        }
                        /** 跨行 则获取跨行对应的位置***/
                        if(rect.rowspan&&rect.rowspan>1){
                            currBottom = rowLines[rect.rowIndex+rect.rowspan].fromPos.top+tableTop+borderTopWidth;//获取行位置处理
                        }
                        rect.left = currLeft;
                        rect.top = currTop;
        ...
        dragEndControlByPageDesignType:function(currField,cfg,isDrag2Table,mousePos4realtive){
            var me = this;
            
            if((!cfg.isSelectArea) &&(!isDrag2Table)){ //非选择区域，拖拽的目的 是为了放置控件
                if(me.data.pageDesignType &&me.data.pageDesignType =='normalForm'){ //查找控件占位
                    var index = me.getControlIdxById(currField.id);
                    me.data.controls.splice(index,1);//先移除再 插入
                    var targetControl = me.findControlByPosOrRect(mousePos4realtive,currField);//在指定控件位置下方放置占位
                    if(targetControl){
            ...
            if(currField.controlType =='page'){ // 页面占位
                if(!currField.otherAttrs.nodeId){//拖拽新增的时候才有

                    currField.otherAttrs.nodeId = $(cfg.el).attr('node-id');
                }
        ...
        updateControlsByPageDesignType:function(currField){
            ...
            if(me.data.pageDesignType &&me.data.pageDesignType =='normalForm'){ //查找控件占位
                ...
                for(var i= 0,len=controls.length;i<len;i++){
                    ...
                    if(curr.style&&curr.style.rect&&curr.style.rect.tableLineId){//在表格布局中 或者数据表格中
                        continue;
                    }
                    me.data.style.currControlZIndex++;
                    ...
                    var fix = top -lastTop;
                    if(curr.htmlType =='tableLine'){//表格布局，修改所有内部控件的位置
        ...
        updateCurrControlStyle:function(){
            ...
            if(currField&&currField.id){
                ...
                if(me.inDetail(currField)){
                    //明细表中的控件需要作为字段
                    var $field = $('#' + currField.id).find('.control-container-pabs');
                    if($field&&$field.length){
                        $field.remove();
                    }
                }else{
                    //非明细表中的控件 需要正常显示控件，只有表单控件才执行更新
                    if(currField.formField || currField.isFormField){ //只有表单控件才更新
                        var html = oui.getById('items').getHtmlByTplId('item-tpl', {item: currField});
                        $('#' + currField.id).replaceWith(html);
                        $('#' + currField.id).addClass('active');
                    }

                }
                if(currField.htmlType =='tableLine' && (!me.isDragging4SelectArea(currField))){
                    //非编辑状态 不用显示操作层
        ...
        dragEndControl:function(cfg){
            ...
            if (isResizer) { //改变大小
                ...
                if (isUpdate) { //update
                    ...
                    //表格控件位置变更后，变更当前表格中所有控件的位置
                    me.updateAllRectsInTableLine(currField);
                    me.dragEndControlByPageDesignType(currField,cfg,isDrag2Table,mousePos4realtive); //根据页面类型进行后置渲染
                    ...
                    //扩展默认设置
                    var defaultField = me.findDefaultControlByBizControlType(currField.controlType,currField.htmlType,currField);
                    ...
                    if (lastId ) { //如果左侧被拖拽的控件有id，则进行取消拖拽的能力
                        $(el).addClass('control-pabs-disable');
                    }
                    var html = oui.getById('items').getHtmlByTplId('item-tpl', {item: currField});
                    var container = oui.getById('items').getEl();
                    $(container).find('.ppt-paper-area').append(html);
                    me.dragEndControlByPageDesignType(currField,cfg,isDrag2Table,mousePos4realtive); //根据页面类型进行后置渲染
                    $('.active','.ppt-paper-area').removeClass('active');
                    $('#' + currField.id).addClass('active');
        ...
        matchEachRule:function(rect,eachRule){
            var inColumn =false;
            var inRow = false;
            if(rect.columnIndex>=eachRule.startColumnIndex &&rect.columnIndex <= eachRule.endColumnIndex){
                inColumn = true
            }else{
                if(rect.columnIndex<eachRule.startColumnIndex){
                    if(rect.colspan &&rect.colspan>1){
                        if((rect.columnIndex+rect.colspan-1)>=eachRule.startColumnIndex){
                            inColumn = true;
                        }
                    }
                }
            }
            if(rect.rowIndex>=eachRule.startRowIndex && rect.rowIndex <= eachRule.endRowIndex){
                inRow = true;
            }else{
        ...
        showOperationArea:function(currField,cfg){
            ...
            var rect = null;
            /** 表格并 当前在选择多格单元格 ****/
            if(currField.htmlType =='tableLine' &&cfg&& cfg.isSelectArea&&(!cfg.isResizer)){
                var dragData = cfg.dragData;
                if(!dragData){
                    //是否选择列或者行
            ...
            $operation.attr('control-pabs-id',currField.id);
            ...
            if(currField.htmlType =='tableLine'){
                ...
                if(!$('.control-line-pabs[control-pabs-id='+currField.id+']').length){
                    $operation.find('.merge-cell').remove();
                    $operation.find('.control-line-pabs').remove();
                    $operation.find('.control-table-num-pabs').remove();
                    var html = oui.getById('items').getHtmlByTplId('control-tableLine-drag-tpl', {item: currField,rect:rect});
        ...
        isTableColumnActive:function(tableLine,columnIndex){
            ...
            if((columnIndex>=me.selectRect.startColumnIndex) && (columnIndex <=me.selectRect.endColumnIndex)){
                flag = true;
            }
            return flag;
        },
        isTableRowActive:function(tableLine,rowIndex){
            ...
            if((rowIndex >=me.selectRect.startRowIndex) && (rowIndex <=me.selectRect.endRowIndex)){
                flag = true;
            }
            return flag;
        },
            ...
            $(document).on('click', '.' + propTitleSelector, function (e) {
                ...
                if (el) {
                    if ($(el).hasClass(activeSelector)) {
                        ...
                        $(el).closest('.'+'design-set-content').find('.'+activeSelector).removeClass(activeSelector);//抽屉模式
                    } else {
                        $(el).closest('.'+'design-set-content').find('.'+activeSelector).removeClass(activeSelector);//抽屉模式
        ...
        renderPaperStyle: function () {
            ...
            if($link4paper&&$link4paper.length){
                $link4paper.replaceWith('<link link-id="page-link"  rel="stylesheet" type="text/css" href="'+me.findThemeUrl()+'" />');
            }else{
                $paper.prepend('<link link-id="page-link"  rel="stylesheet" type="text/css" href="'+me.findThemeUrl()+'" />');
            }
            ...
            var $paperOuter = $('.ppt-paper-area-outer');
            $paperOuter.css({ //处理外框宽度高度
                width: (me.data.style.width+100) + me.data.style.cellType,
                height:( me.data.style.height+100) + me.data.style.cellType
            });
            $paper.css({
                // 'background-image': me.data.backgroundImage?('url('+me.data.backgroundImage+')'):'',
                width: me.data.style.width + me.data.style.cellType,
                height: me.data.style.height + me.data.style.cellType
            });
            var defaultTopFixed = 18 + $paper[0].offsetTop;
            var defaultLeftFixed = 18 + $('.ppt-paper-area').offset().left- $('.design-middle').offset().left ;
            ...
            console.log(left,12333333333333333)
        },
        ...
        changed4propsRealTime:function(cfg,callback){
            ...
            if(!document.getElementById(currentControl.id)){
                //需要重新渲染页面
                me.state ='down2selectPage';
            }
            if ('init,down2selectPage'.split(',').indexOf(me.state) > -1) {
                me.renderPaperStyle();
                //改变全局控件属性
                if(cfg&&cfg.el){
                    var prop = $(cfg.el).attr('bindprop')||$(cfg.el).attr('invoke-prop')||"";
                    if(prop){
                        if(prop.indexOf('currentControl')>=0){
                            //对控件全局属性进行处理
                            try{
                                me.changeAllControls(prop,oui.JsonPathUtil.getJsonByPath(prop,me.data));
                            }catch(err){}
                            me.renderCenterAll();
                        }
                        //页面宽高调整导致的拖拽事件变更
                ...
                if (me.data.currentControl == currentControl) {//当前对象则执行相关逻辑
                    callback&&callback();
                } else {
                    me.data.tempData = {};
                ...
                if (currentControl.id) {
                    ...
                    var dragStatus = document.getElementById(currentControl.id).getAttribute('drag-status');
                    oui.clearByContainer('#'+currentControl.id);
                    oui.clear4notUse();
                    var html = oui.getById('items').getHtmlByTplId('center-control-tpl', {item: currentControl});
                    me.patchCurrControl(currentControl,html);
                    document.getElementById(currentControl.id).setAttribute('drag-status',dragStatus);
                    oui.parse();
                    setTimeout(function () {
                        if (me.data.currentControl == currentControl) {//当前对象则执行相关逻辑
        ...
        changedControlProp:function(control,key){
            ...
            if(key.indexOf('style.')==0){ //外框样式属性调整
                me.findControlStyle4Dom(control,control.parentId?true:false);
            }else if(key.indexOf('innerStyle.styleInnerOuter.')==0){//控件内框样式
                me.findControlStyleInnerOuter4Dom(control,control.parentId?true:false);
            }else if(key.indexOf('innerStyle.styleTitle.')==0){//控件标题样式
                me.findControlInnerStyleTitle4Dom(control,control.parentId?true:false);
            }else if(key.indexOf('innerStyle.styleFieldOuter.')==0){//控件字段外框样式
                me.findStyleFieldOuter4Dom(control,control.parentId?true:false);
            }else if(key.indexOf('innerStyle.styleField.')==0){//控件字段值样式
        ...
        patchCurrControl:function(currentControl,html){
            if(oui.browser.ie || oui.browser.isEdge){
                if (currentControl.htmlType == ControlTypeEnum.selectArea.name) {
                    ...
                    var selectAreaBoxHtml = oui.getById('items').getHtmlByTplId('selectArea-box-tpl', {item: currentControl});
                    document.getElementById(currentControl.id+'_box').outerHTML = $.trim(selectAreaBoxHtml);
                ...
                if (currentControl.htmlType == ControlTypeEnum.selectArea.name) {
                    ...
                    oui.getById('absoluteProps').patchDom(document.getElementById(currentControl.id),$.trim(html));

                    var selectAreaBoxHtml =oui.getById('items').getHtmlByTplId('selectArea-box-tpl', {item: currentControl});
                    oui.getById('absoluteProps').patchDom(document.getElementById(currentControl.id+'_box'),$.trim(selectAreaBoxHtml));
        ...
        findBorderWidth: function (currField, bindPropKey) {
            var me = this;
            var arr = [me.hasBorderLeft(currField, bindPropKey), me.hasBorderTop(currField, bindPropKey), me.hasBorderRight(currField, bindPropKey), me.hasBorderBottom(currField, bindPropKey)];
            arr.sort();
            var border = arr[arr.length - 1];
            return border || 0;
        },
        findBorderRadius:function(currField, bindPropKey){
            var objKey = bindPropKey.replace('tempControl.', '').replace('._borders', '');
        ...
        findBorderColor: function (currField, bindPropKey) {
            var objKey = bindPropKey.replace('tempControl.', '').replace('._borders', '');
            var style = oui.JsonPathUtil.getJsonByPath(objKey, currField) || {};
            var color = style.borderLeftColor || style.borderTopColor || style.borderRightColor || style.borderBottomColor;
        ...
        changed4setBorderStyle: function (cfg) {
            ...
            var objKey = bindPropKey4Style.replace('._borderStyle', '').replace('tempControl.', '');
            var borderStyle = oui.JsonPathUtil.getJsonByPath(bindPropKey4Style, me.data) || 'solid';
        ...
        changed4setBorderColor:function(cfg){
            ...
            var objKey = bindPropKey4Color.replace('._borderColor', '').replace('tempControl.', '');
            var color = oui.JsonPathUtil.getJsonByPath(bindPropKey4Color, me.data) || '';
            var styleObj = oui.JsonPathUtil.getJsonByPath(objKey, currentControl) || {};
        ...
        changed4setBorderWidth: function (cfg) {
            ...
            var objKey = bindPropKey4Width.replace('._borderWidth', '').replace('tempControl.', '');

            var _borderWidth = oui.JsonPathUtil.getJsonByPath(bindPropKey4Width, me.data) || 0;
            var styleObj = oui.JsonPathUtil.getJsonByPath(objKey, currentControl);
            if (_borderWidth) {
                ...
                styleObj.borderBottomWidth && (styleObj.borderBottomWidth = _borderWidth);
            } else {
            ...
            if ('init,down2selectPage'.split(',').indexOf(me.state) > -1) {
                me.changed4propsRealTime(cfg, function () {
                    try{
                        me.changeAllControls(objKey+'.borderLeftWidth',styleObj.borderLeftWidth);
                        me.changeAllControls(objKey+'.borderTopWidth',styleObj.borderTopWidth);
                        me.changeAllControls(objKey+'.borderRightWidth',styleObj.borderRightWidth);
                        me.changeAllControls(objKey+'.borderBottomWidth',styleObj.borderBottomWidth);
                        me.renderCenterAll();
                        //me.setCurrPropsData4page('props','down2selectPage');
                    }catch(err){}
                });
        ...
        changed4setBorders: function (cfg) {
            ...
            var objKey = bindProp.replace('tempControl.', '').replace('._borders', '');
            var bindProp4Width = bindProp.replace('._borders', '._borderWidth');
            var currStyleObj = oui.JsonPathUtil.getJsonByPath(objKey, currentControl) || {};
            ...
            if (borders && borders.length) {
                ...
                for (var i = 0, len = borders.length; i < len; i++) {
                    ...
                    currStyleObj[borderKey] = last[borderKey] || me.findBorderWidth(currentControl, bindProp) || 1;
                }
            } else {
                /** 选择框 需要特殊处理*****/
            ...
            if ('init,down2selectPage'.split(',').indexOf(me.state) > -1) {
                if(bindProp.indexOf('tempControl')>=0){
                    try{
                        me.changeAllControls(objKey+'.borderLeftWidth',currStyleObj.borderLeftWidth);
                        me.changeAllControls(objKey+'.borderTopWidth',currStyleObj.borderTopWidth);
                        me.changeAllControls(objKey+'.borderRightWidth',currStyleObj.borderRightWidth);
                        me.changeAllControls(objKey+'.borderBottomWidth',currStyleObj.borderBottomWidth);
                        me.renderCenterAll();
                        me.setCurrPropsData4page('props','down2selectPage');
                    }catch(err){}
                }
        ...
        hasBorder: function (currField, bindPropKey) {
            ...
            if ((!currField) || (!currField.style)) {
                ...
                flag = me.hasBorderLeft(currField, bindPropKey) || me.hasBorderTop(currField, bindPropKey) || me.hasBorderRight(currField, bindPropKey) || me.hasBorderBottom(currField, bindPropKey);
        ...
        hasBorderByProp: function (currField, bindPropKey, prop) {
            var me = this;
            var objKey = bindPropKey.replace('._borders', '').replace('tempControl.', '');
            var widthKey = bindPropKey.replace('._borders', '._borderWidth');
            var styleObj = currField && oui.JsonPathUtil.getJsonByPath(objKey, currField);
            var border = currField && styleObj && styleObj[prop];
            if (!border) {
                var tempBorder = oui.JsonPathUtil.getJsonByPath(bindPropKey, me.data) || "";
                if (tempBorder.indexOf(prop) >= 0) {
                    return border || oui.JsonPathUtil.getJsonByPath(widthKey, me.data) || 1;
                }
            }
            return border;
        },
        ...
        event2editTable:function(cfg){
            ...
            var $ctrl = $(cfg.el).closest('.control-pabs-component');
            var id = $ctrl.attr('id');
            if(!id){
                return ;
            }
            ...
            if(currField.style.isDragging4SelectArea){ //判断当前表格控件 是否是编辑状态
        ...
        findFieldHtml:function(control,render4tpl){
            ...
            return me.plugin.PageDesignControlAdapter.render(control,render4tpl)||'';
        },
        /** 清空页面背景****/
        ...
        event2cloneControl: function (cfg) {
            ...
            var $control = $(cfg.el).closest('.control-pabs-component');
            ...
            if($(cfg.el).hasClass('design-copy-ctrl-right')){ //向右复制
            ...
            if(newControl.htmlType =='tableLine'){
                ...
                for(var i in cellsMap){
                    ...
                    if(rect&&rect.controlId&&rect.tableLineId){
                        ...
                        if(childControl){
                            var newChildControl = oui.parseJson(oui.parseString(childControl));
                            ...
                            var tempHtml = oui.getById('items').getHtmlByTplId('item-tpl', {item: newChildControl});
                            childHtmls.push(tempHtml);
                        }else{
                            me.removeRectInTableLineByColumnAndRow(newControl,rect.columnIndex,rect.rowIndex);
                        }
                    }
                }
            ...
            var html = oui.getById('items').getHtmlByTplId('item-tpl', {item: newControl});
            var container = oui.getById('items').getEl();
            $(container).find('.ppt-paper-area').append(html+childHtmls.join(''));
            $('.active','.ppt-paper-area').removeClass('active');
        ...
        event2removeControl: function (cfg) {
            ...
            setTimeout(function(){
                ...
                $('#' + id,'.ppt-paper-area').remove();
                if(disableSelector.length){
                    $(disableSelector.join(','),'.design-left').removeClass('control-pabs-disable');
                }
        ...
        setCurrPropsData4page:function(type,state){
            ...
            me.setCurrPropsData(me._globalPageData, type||'center', state||'down2selectPage');
        ...
        event2showCutImg: function (cfg) {
            ...
            oui.uploadURL = oui.uploadURL || me.paramCfg.uploadUrl||oui.getTop().oui.uploadURL;
            ...
            oui.showCutImg({
                ...
                boxHeight: 300,
                showPreview: true,
                panelStyle: 'width:' + $scrollEl.width() + 'px;height:' + $scrollEl.height() + 'px',
                confirm: function (base64, boxData, o) {
                    oui.uploadBase64(base64,function(result){
                        ...
                        if(!propKey){ //默认改变页面背景
                            ...
                            //otherAttrs.imgName
                            //ohterAttrs.img
                            oui.JsonPathUtil.setObjByPath(propKey+'Id',me.data,imgId,true); //设置图片Id
                            oui.JsonPathUtil.setObjByPath(propKey+'Name',me.data,imgName,true);//设置图片名称
                            oui.JsonPathUtil.setObjByPath(propKey,me.data,previewUrl,true);//设置图片url
                            me.changed4props({},function(){
                                me.setCurrPropsData(me.data.currentControl, 'center', 'down2selectField');
                            });
                        }
        ...
        event2selectBoxArea: function (cfg) {
            ...
            if (me.hasSelectBox()) {//取消选择区域
                ...
                currField.style.width4mm = width4mm;
                currField.style.height4mm = height4mm;
                var left = $scrollEl[0].scrollLeft + ($scrollEl.width() - currField.style.width) / 2 - $paper[0].offsetLeft;
                var top = $scrollEl[0].scrollTop + ($scrollEl.height() - currField.style.height) / 2 - $paper[0].offsetTop;
                left = parseInt(left);
                top = parseInt(top);
                if (left < 0) {
                    left = 0;
        ...
        event2preview: function () {
            var me = this;
            var url =oui.getContextPath() + 'res_engine/ppt_design/pc/ppt-runtime.html';
        ...
        checkSaved:function(ok,cancel,title){
            var me = this;
            if(me.hasChange){
                oui.getTop().oui.confirmDialog(title||'当前页面没保存',function(){
                    ok&&ok();
                },function(){
                    cancel&&cancel();
                });
        ...
        event2export:function(){
            ...
            var blob = new Blob([oui.parseString(data)], {type: "text/plain;charset=utf-8"});
            saveAs(blob, data.name+".json");
        },
        event2triggerPaperDown: function (cfg) {
            $('.ppt-paper-area').trigger('mousedown');
        },
        down2selectPaperArea: function (cfg) {
            var me = this;
            if (!$(cfg.e.target).is('.ppt-paper-area')) {
                return;
            }
            me.currAccordingActiveId='';
            //选中当前页，并进行属性渲染
            ...
            if(me.data.currentControl &&me.data.currentControl.htmlType =='tableLine'){
                if(me.data.currentControl.style.isDragging4SelectArea){
                    me.data.currentControl.style.isDragging4SelectArea = false;
                    var $ctrl = $('#'+me.data.currentControl.id,'.ppt-paper-area');
                    $ctrl.removeClass('drag-select-area');
        ...
        findSelectAreaItems: function () {
            ...
            var boxRight = selectBox.style.left + selectBox.style.width+selectBox.style.borderRightWidth;
            var boxBottom = selectBox.style.top + selectBox.style.height+selectBox.style.borderBottomWidth;
            ...
            for (var i = 0, len = items.length; i < len; i++) {
                ...
                if (curr == selectBox) {
                    ...
                    if ((currRight > boxLeft && currBottom > boxTop) &&
                        (currLeft < boxRight && currBottom > boxTop) &&
                        (currTop < boxBottom)) {
                        //在区域内
                        var temp = $.extend(true, {}, curr);
                        temp.style.left -= boxLeft;
        ...
        findSelectAreaHtml: function (render4tpl) {
            ...
            if (!me._renderSelectAreaHtml) {
                me._renderSelectAreaHtml = template.compile(document.getElementById('items-select-tpl').innerHTML);
            }
        ...
        findContentAreaHtml: function (render4tpl) {
            ...
            if (!me._renderContentAreaHtml) {
                me._renderContentAreaHtml = template.compile(document.getElementById('items-content-tpl').innerHTML);
            }
        ...
        savePageDesign:function(callback){
        debugger;
            var me = this;
            var data = me.getDesignData();
            me.paramCfg.page = data;
            var params = oui.getParam();
            me.updateParamCfgSource(me.paramCfg);//更新调用处参数
            me.hasChange = false;
            //保存页面数据

            var one = oui.findOneFromArrayBy(me.pageList||[],function (item) {
                if(item.PageId == me.pageId){
                    return true;
                }
            });
            // 保存 页面定义
            if (one.PageImgUrl && one.PageImgUrl.startsWith("data:image")) {
                oui.ajaxPost(oui.pptAjaxConfig.updatePageImgUrl, {
                    PageId: me.pageId,
                    TranslateId: me.pptId,
                    PageImgUrl: one.PageImgUrl,
                }, function (res) {
                    one.PageImgUrl = res.PageImgUrl;
                    oui.ajaxPost(oui.pptAjaxConfig.savePageUrl,{
                        PageId:me.pageId,
                        TranslateId:me.pptId,
                        PageName:one.PageName,
                        TemplateId:one.TemplateId||'',
                        PageImgUrl:res.PageImgUrl||'',
                        PageData:oui.parseString(data)
                    },function(res){
                        callback&&callback(res,data);
                    });
                },function (res) {
                    console.log("base64图片保存失败")
                });
            } else {
                oui.ajaxPost(oui.pptAjaxConfig.savePageUrl,{
                    PageId:me.pageId,
        ...
        printFormData:function(){
            ...
            oui.postData(url,{
                ...
                if(res.success){
                    ...
                    var content = '<link href="' + oui.getContextPath() + 'res_engine/ppt_design/pc/css/ppt-common.css" type="text/css" rel="stylesheet"/>';
                    content+='<style type="text/css"> .ppt-paper-area {overflow:hidden;} .control-pabs{position:absolute;}</style>';
                    content += html;
                    //oui.printIt({
                    //    content: content
                    //});
        ...
        initEnd: function () {
            ...
            setTimeout(function(){
                ...
                oui.require([oui.getContextPath()+'res_common/third/watch/watch.js'
                    ...
                    WatchJS.watch(me,'data',function(prop, action, newvalue, oldvalue) {
                        ...
                        var fun = function(){
                            try{
                                me.update4watch&&me.update4watch(); //根据监听来更新
                                me.updateTimer = null;
                            }catch(err){
                            }
                        };
        ...
        getSrcAndLink:function(type,num){
            return new Promise(function(reslove,reject){
                var param = 'intMaterialType='+type+'&num='+num+'';
                var d = top.dialog({
                    id: 'DialogFileManager',
                    padding: 0,
                    title: "文件管理器",
                    url: oui.getContextPath()+'Views/Dialog/DialogFileManager.aspx?' + param,
                    onclose: function () {
                        reslove(this.returnValue)
                    }
                }).showModal();
            });
        },
        // js调用本地文件选择文件资源，回调处理相关参数
        showFileDialog4local:function(config){
            var id = 'file_local_select';
            var $file = $('#'+id);
            if($file&&$file.length){
                $file.remove();
            }
            var me = this;
            $('body').append('<input id='+id+' type="file" />');
            $('#'+id).on('change',function(e){
                console.log('file  change',e);
                var files = me.getChangeFiles(e); 
                //TODO 值改变事件 触发 文件处理
                config.callback&&config.callback(files);
                setTimeout(function(){
                    $('#'+id).off('change');
                    $('#'+id).remove();
                },1000);
        ...
        getChangeFiles:function(e){
            var files = e.target.files||[];
            var arr = [];
            oui.eachArray(files,function(rawFile){
                arr.push({
                    name: rawFile.name,
                    size: rawFile.size,
                    percentage: 0,
                    uid: 'f_'+oui.getUUIDLong(),
                    raw: rawFile      
                });
            });
            return arr;
        },
        showFileDialog:function (config) {
            if(config.local){
                this.showFileDialog4local(config);
                return;
            }
            var me = this;
            oui.require([
                oui.getContextPath()+'Views/productEdit/public/stylesheets/quill.css',
                oui.getContextPath()+'Views/productEdit/public/javascripts/quill.js',

                oui.getContextPath()+'Scripts/artdialog/ui-dialog.css',
                oui.getContextPath()+'Views/productEdit/public/javascripts/dialog-min.js',
                oui.getContextPath()+'Scripts/artdialog/dialog-plus-min.js',
                oui.getContextPath()+'Views/productEdit/public/javascripts/product.js',
                oui.getContextPath()+'Views/productEdit/public/javascripts/product_main.js',
                oui.getContextPath()+'Views/productEdit/public/javascripts/product_active.js',
                oui.getContextPath()+'Views/productEdit/public/javascripts/product_util.js',
            ],function(){
                me.getSrcAndLink(config.fileType,config.num).then(function(res){
                    var files = me.transFiles(res,config.fileTypes||[]);
                    config.callback&&config.callback(files);
                    return res;
                });
            });
        },
        /** 获取枚举类型  1视频，2图片，3音频，4文档 */
        getFileTypeEnums:function(){
             if(!this._fileTypeEnums){
                 this._fileTypeEnums = {
                     video:{
                         name:'video',
                         value:1,
                         transFile:function (item) {
                             var obj = {
                                 fileType:this.value,//文件类型
                                 coverImgUrl:item.CoverImgOssPath, //视频封面图
                                 url:item.FileOSSPath, //资源路径
                                 resolution:item.Resolution, //分辨率
                                 fileName:item.FileName,// 文件名
                                 fileSize:item.FileSize,//文件大小
                                 fileDuration:item.FileDuration//视频时长
                     ...
                     music:{
                         ...
                         transFile:function (item) {
                             var obj = {
                                 fileType:this.value,//文件类型
                                 coverImgUrl:item.CoverImgOssPath, //音乐封面图
                                 url:item.FileOSSPath, //资源路径
                                 fileName:item.FileName,// 文件名
                                 fileSize:item.FileSize,//文件大小
                                 fileDuration:item.FileDuration//视频时长
                     ...
                     doc:{
                         ...
                         transFile:function (item) {
                             var arr = [];
                             if(item.FileImgList){
                                 arr = item.FileImgList.split(',');
                             }
                             var obj = {
                                 fileType:this.value,//文件类型
                                 coverImgUrl:item.CoverImgOssPath, //文档封面图
                                 url:item.FileOSSPath, //资源路径
                                 fileName:item.FileName,// 文件名
                                 fileSize:item.FileSize,//文件大小
                                 fileImgList:arr//文档图片列表
                             };
                             return obj;
                         }
                     }
             ...
             return this._fileTypeEnums;
        },
        findFileTypeEnumByValue:function(fileType){
            var fileEnums = this.getFileTypeEnums();
            for(var k in fileEnums){
                if(fileEnums[k] && (fileEnums[k].value == fileType)){
                    return fileEnums[k];
                }
            }
            return null;
        ...
        transFiles:function(res,fileTypes){
            var me = this;
            var arr = [];
            //todo 转换 成我们的数据结构
            oui.eachArray(res, function (item) {
                var itemData = oui.parseJson(item.item);
                var fileTypeEnum = me.findFileTypeEnumByValue(itemData.MaterialType);
                var curr= null;
                if(fileTypeEnum &&fileTypeEnum.transFile){
                    if(fileTypes){
                        if(fileTypes.indexOf(fileTypeEnum.name)>-1){
                            curr = fileTypeEnum.transFile(itemData);
                        }
                    }else{
                        curr = fileTypeEnum.transFile(itemData);
                    }
