!(function (win, $) {
    ...
    var AbsoluteDesign = {
        ...
        styleStringKeys:('' +
        'backgroundImage,backgroundColor,backgroundRepeat,backgroundSize,' +
        'verticalAlign,' +
        'borderLeftColor,borderTopColor,borderBottomColor,borderRightColor,' +
        'borderLeftStyle,borderTopStyle,borderBottomStyle,borderRightStyle,' +
        'color,fontFamily,fontWeight,textAlign,whiteSpace').split(','),
        ...
        ***/
        init: function () {
            ...
            oui.loadApiConfig(
            oui.getContextPath()+"res_engine/portal_design/pc/js/api-config.json",
            "oui.biz",function(){
                //根据url参数初始化 输入参数
                me.initByUrlParams(function(){
                    var param = oui.getParam();
                    var loadProjectPortalUrl = param.loadProjectPortalUrl;
                    oui.postData(loadProjectPortalUrl,{},function(res){
                        me.data = res.portal;
                        ...
                        me.loadPageQueryTemplatesUrl = me.data.extraAttrs.loadPageQueryTemplatesUrl;
                        me.updateProjectPortalUrl = me.data.extraAttrs.updateProjectPortalUrl;
                        me.init4parse();
                    },function(res){
                    },'加载中...'); 
        ...
        initByUrlParams:function(callback){
            ...
            var designTplUrl = oui.getContextPath()+'res_engine/portal_design/common/portal-design-tpl.html';
            if(pageDesignType =='printForm'){
                //纯div占位 模板
                designTplUrl = oui.getContextPath()+'res_engine/portal_design/common/portal-design-tpl-space.html';
            }
            ...
            var bizJsArr = [oui.getContextPath()+'res_engine/portal_design/common/js/portal-design.js'];//默认公共
        ...
        bindDrag: function (options) {
            ...
            $doc.on('mousedown', options.dragSelector, function (e) {
                ...
                var mouseup = function (ev) {
                    ...
                    //        $('#canvas_box').remove();
                    //        var html='<div id="canvas_box" style="display:none;" class="portal-paper-area-canvas" style="width:'+$dropContainer[0].scrollWidth+'px;height:'+$dropContainer[0].scrollHeight+'px"></div>';
        ...
        event2merge:function(cfg){
            ...
            if(rects.length==1){
                ...
                //四 更新单元格中的控件
                var controlEl = $('#'+controlInCell.id,'.portal-paper-area')[0];
        ...
        event2split:function(cfg){
            ...
            if(controlIds&&controlIds.length){
                ...
                $(selector,'.portal-paper-area').each(function(){
        ...
        buildHtmlCanvasByControlId: function (id, callback) {
            ...
            var $paperArea = $('.portal-paper-area');
        ...
        removeControlsByIds:function(ids){
            ...
            $(selector,'.portal-paper-area').remove();
        ...
        bindDragEvents:function(){
            ...
            this.bindDrag({
                ...
                dropContainer: '.portal-paper-area',//放置元素区域的容器
                down: function (cfg) {
                    ...
                    if (isUpdate) {
                        ...
                        //拖拽的是操作层
                        //不同层做高亮
                        $('.active','.portal-paper-area').removeClass('active');
                        $(el).addClass('active');
        ...
        dragEndControl:function(cfg){
            ...
            if (isResizer) { //改变大小
                ...
                if (isUpdate) { //update
                    ...
                    $(container).find('.portal-paper-area').append(html);
                    me.dragEndControlByPageDesignType(currField,cfg,isDrag2Table,mousePos4realtive); //根据页面类型进行后置渲染
                    $('.active','.portal-paper-area').removeClass('active');
                    $('#' + currField.id).addClass('active');
        ...
        changed4propsRealTime:function(cfg,callback){
            console.log("res_engine\portal_design\js\portal-design.js")
            var me = this;
            var currentControl = me.data.currentControl;
        ...
        event2cloneControl: function (cfg) {
            ...
            var container = oui.getById('items').getEl();
            $(container).find('.portal-paper-area').append(html+childHtmls.join(''));
            $('.active','.portal-paper-area').removeClass('active');
        ...
        event2preview: function () {
            ...
            var me = this;
            var url =oui.getContextPath() + 'res_engine/portal_design/pc/portal-runtime.html';
        ...
        event2runForm:function(){
            ...
            var win = oui.openWindow({
                url: oui.getContextPath() + 'res_engine/portal_design/pc/portal-preview.html',
                //openType:'_blank',
                title: '预览'
            });
            win.bodyHtml = html;
        ...
        event2triggerPaperDown: function (cfg) {
            $('.portal-paper-area').trigger('mousedown');
        },
        down2selectPaperArea: function (cfg) {
            var me = this;
            if (!$(cfg.e.target).is('.portal-paper-area')) {
                return;
            }
            me.currAccordingActiveId='';
            //选中当前页，并进行属性渲染
            ...
            if(me.data.currentControl &&me.data.currentControl.htmlType =='tableLine'){
                if(me.data.currentControl.style.isDragging4SelectArea){
                    ...
                    var $ctrl = $('#'+me.data.currentControl.id,'.portal-paper-area');
                    $ctrl.removeClass('drag-select-area');
        ...
        printFormData:function(){
            ...
            oui.postData(url,{
                ...
                if(res.success){
                    ...
                    var content = '<link href="' + oui.getContextPath() + 'res_engine/portal_design/pc/css/portal-common.css" type="text/css" rel="stylesheet"/>';
                    content+='<style type="text/css"> .portal-paper-area {overflow:hidden;} .control-pabs{position:absolute;}</style>';
                    content += html;
                    //oui.printIt({
                    //    content: content
                    //});
