...
(function(){
	...
	var FlowUi = {	
		...
		render:function(id,notUpdateEl){
			...
			var html=this.tpls[id](this.getData.call({},key));
	...
	FlowUi = oui.biz.Tool.crateOrUpdateClass(FlowUi);
	...
	FlowUi.rect = function(o, r) {
		...
		//$([_rect.node, _text.node, _name.node, _img.node]).attr('oui-e-mousedown','mousedown2contextMenu');
		
		$([_rect.node, _text.node, _name.node, _img.node]).attr({
			nodeId:_o.id,
			'oui-e-contextmenu':'mousedown2contextMenu'
		});
		$([_text.node]).attr({
	...
	FlowUi.path = function(o, r, from, to) {
		...
		var rectresizeHandler = function(e, src) {
			...
			if (_from && _from.getId() == src.getId()) {
				...
				var p = FlowUi.util.connPoint(_from.getBBox(), rp);
			...
			if (_to && _to.getId() == src.getId()) {
				...
				var p = FlowUi.util.connPoint(_to.getBBox(), rp);
		...
		function refreshpath() {
			...
			_text.attr({
						x : (mid.x||0) + (_textPos.x||0),
						y : (mid.y||0) + (_textPos.y||0)
					});
			// $('body').append('refresh.');
		}
	...
	FlowUi.props = function(o, r) {
		var _this = this, _pdiv = $('#ouiflow_props').hide().draggable({
					handle : '#ouiflow_props_handle'
				}).resizable().css(FlowUi.config.props.attr).bind('click',
				function() {
					return false;
	...
	FlowUi.init = function(c, o) {
		...
		var _w = $(window).width(), _h = $(window).height(), _r = Raphael(c,o.width*1.2, 
		o.height*1.2), _states = {}, _paths = {};
		...
		$(document).click(function() {
					...
					$(_r).trigger('showprops', [FlowUi.config.props.props, {
										getId : function() {
											return '00000000';
										}
									}]);
		...
		$(_r).bind('addrect', function(e, type, o) {
			// $('body').append(type+', ');
			var rect = new FlowUi.rect($.extend(true, {},
							FlowUi.config.tools.states[type], o), _r)
			_states[rect.getId()] = rect;
		});
		// 添加路径
		var addpathHandler = function(e, from, to) {
			var path = new FlowUi.path({}, _r, from, to);
		...
		if (FlowUi.config.editable) {
			// 工具栏
			$("#ouiflow_tools").draggable({
						handle : '#ouiflow_tools_handle'
					}).css(FlowUi.config.tools.attr);

			$('#ouiflow_tools .node').hover(function() {
						$(this).addClass('mover');
					}, function() {
						$(this).removeClass('mover');
					});
			$('#ouiflow_tools .selectable').click(function() {
			...
			$('#ouiflow_tools .state').each(function() {
						$(this).draggable({
									helper : 'clone'
								});
					});
			...
			$('#ouiflow_save').click(function() {// 保存
						var data = '{states:{';
						...
						if (data.substring(data.length - 1, data.length) == ',')
							data = data.substring(0, data.length - 1);
						data += '},props:{props:{';
						for (var k in FlowUi.config.props.props) {
							data += k + ":{value:'"
									+ FlowUi.config.props.props[k].value
									+ "'},";
						}
						if (data.substring(data.length - 1, data.length) == ',')
							data = data.substring(0, data.length - 1);
						...
						FlowUi.config.tools.save.onclick(data);
						// alert(data);
					});
