...
(function(win){
    oui.NS('oui.form');
    var FlowBiz4formDefine = $.extend(true,{
        prefix:'flow',
        bizParam:{},
        getBiz: function () {
            return this;
    ...
    oui.form.FlowBiz4formDefine = FlowBiz4formDefine;
    /**
     * 保存流程成功回调
     * @param data
     */
    FlowBiz4formDefine.saveWorkFlowSuccess = function(data,cfg,success,error){
        success&&success(msg);
        if($(cfg.el).attr('saveType') =='save'){
            var urlParam = {
                moduleType:'formDefine',
                category:oui.getParam("category"),
                moduleBizPath:true,
                _t:new Date().getTime(),
            };
            urlParam.tipMsg = 'SAVE_SUCCESS';
            var workflowUrl = data.workflowUrl;
            var url = workflowUrl+'&'+ $.param(urlParam);
            oui.go(url,{},true);
        }
    };
    /**
     ...
     * @param data
     */
    FlowBiz4formDefine.saveWorkFlowError = function(data,cfg){
        var _self = this;
        console.log('保存流程失败');
        console.log(data);
        oui.alert(data);
     ...
     * 保存工作流
     */
    FlowBiz4formDefine.saveWorkFlow=function(cfg){
        var iframe = $('#flow-ui-workflow');
        var win = iframe[0].contentWindow;
        var saveType = $(cfg.el).attr('saveType');
        if(saveType=='save4test'){
            win.$("#flow-ui-bottomButtons").find('[oui-e-click=saveWorkFlow][saveType=save4test]').attr('saveWorkFlowCallbackBiz','window.parent.oui.form.FlowBiz4formDefine');
            win.$("#flow-ui-bottomButtons").find('[oui-e-click=saveWorkFlow][saveType=save4test]').trigger('click');
        }else{
            win.$("#flow-ui-bottomButtons").find('[oui-e-click=saveWorkFlow][saveType=save]').attr('saveWorkFlowCallbackBiz','window.parent.oui.form.FlowBiz4formDefine');
            win.$("#flow-ui-bottomButtons").find('[oui-e-click=saveWorkFlow][saveType=save]').trigger('click');
        }
    };
    /**
     * 事件名支持，在模板中使用
     * @type {{click: string}}
     */
    FlowBiz4formDefine.Events= {
        click:'click'
    };
    /**
     * 初始化
     */
    FlowBiz4formDefine.init = function(param){
        var _self = this;
        _self.isTest = oui.getParam('isTest');
        template.helper('FlowBiz4formDefine',_self);
        //_self.render('flow-ui-bottomButtons');
        var tipsMsg = oui.getParam('tipMsg');
        tipsMsg&&oui.showAutoTips(tipsMsg=='SAVE_SUCCESS'?'保存成功':tipsMsg);
        _self.flowId  = param.flowId;
        _self.flowJson = param.flowJson || ""; //字符串传入
        win.FlowBizModuleController ='oui.form.FlowBiz4formDefine';

    };
    /**
     * 获取工作流对象,流程图初始化前传入到流程图页面中
     */
    FlowBiz4formDefine.getWorkFlowJson = function(){
        var _self = this;
        return _self.flowJson;
    };
    /**
     * 获取流程所属模块的信息
     */
    FlowBiz4formDefine.getWorkFlowJson4String = function(){
        var _self = this;
        return  oui.parseString(_self.flowJson);
    };
    FlowBiz4formDefine.getWorkFlowFromServer = function(success,error){
        var formId = oui.getParam('moduleId');
        var contextPath = oui.getContextPath();
        var formWorkFlowUrl = oui.getPageParam('formWorkFlowUrl');
        oui.getData(formWorkFlowUrl, {}, function (result) {
            if (result.success) {
                var msg = oui.parseJson(result.msg);
                success&&success(msg.workFlowJSON);
            } else {
                error&&error(msg);
            }
        });
    ...
    /**
     * oui.storage.get("formName4workflow")
     oui.storage.get("moduleBizPath")

     杨海 2016/9/9 2:01:39
     oui.storage.remove("moduleBizPath")
     * 业务模块与流程的集成配置
     * @type {{moduleId, moduleType: string, name}}
     */
    var	workFlowJson ={
        moduleId:oui.getParam('moduleId'),
        moduleType:'formDefine',
        name: oui.storage.get("formName4workflow")
    };
    oui.setPageParam('workFlowJson',workFlowJson);
})(window);
