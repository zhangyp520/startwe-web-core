!(function(win, $) {
  ...
  var AbsoluteDesign = {
    ...
    init4parse: function() {
      ...
      if(me.isReload == false ) me.loadUrl() 
    ...
    updateParamCfgSource: function(paramCfg) {
      ...
      if (param.ouiInWindowDialog && param.ouiInWindowDialog + "" == "true") {
        ...
        console.log('window参数更新了：',oui.parseString(paramCfg));
    ...
    initByUrlParams: function(callback) {
      ...
      var designTplUrl =
        oui.getContextPath() +
        "res_engine/page_design/common/page-design-tpl.html";
      ...
      if (paramCfg.scriptPkg && !me.isReload) {
        //设置自定义命名空间
        var plugin = {
      ...
      if(!me.saveUrl &&(paramCfg&&paramCfg.params)){
        me.saveUrl = paramCfg.params.saveUrl;
      }
        oui.require(
          bizJsArr,
          function() { 
    ...
    init4static: function() { 
      var me = this;
      /** 页面属性配置   ****/
      me.pageKeys = "id,pageId,name,bizId,projectId,appId,moduleId,enName,modelType,version,canCloneControl,backgroundImageId,backgroundImageName,backgroundImage,description,pageDesignType,listContent,content,selectContent,style,innerStyle,controls,otherAttrs,events".split(
        ","
      ); 
      /** 主题模板配置 与扩张 **/
      me.themeTypes = [
    ...
    //载入新page
    reloadPage:function(newpage){
      $('#control_items').remove(); 
     
      var me = this
      me.newPageData = newpage 
     // me.data = newpage;   

      me.isReload =  true
      
      $('.paper-area-outer').html('<oui-view id="items" useVDom="false" data="com.oui.absolute.AbsoluteDesign.getCurrPropsData()"></oui-view>');
      ...
      //     bizJsArr,
      //     function() {  
      //          me.plugin = com.oui.absolute.AbsoluteDesign.plugin 
      //     },
      //     function() {
      //       oui
      //         .getTop()
    ...
    initBefore: function() {
      ...
      var page = me.newPageData || params.page || {};
    ...
    //tabmenu 点击事件
    event2clickMenuTab:function (cfg){
        var cssStyle = $(cfg.el).attr('data-active-style');
        $(cfg.el).parent().find('.active-bar')[0].style = cssStyle;

        $(".action-tabs .action-tabs-item").removeClass("current")
        $(cfg.el).addClass("current");
        $('.left-list').hide();
        let type = $(cfg.el).data("type");
        $("." + type + '-list').show();
    },
    //左侧菜单点击事件
    event2clickLeftSideMenuTab:function (cfg){
      var me = this;
      var $el =$(cfg.el);
    ...
    showPage:function (){
      ...
      $(".right-detail").show();
      $(".design-area").css("width","calc(100% - 282px - 232px)");
    },
    showLogicPage:function (){
      $('.page-design-content').hide();
      $('.page-action-area').hide();
      $('.logic-content').show();
      $('.logic-action-area').show();
      $(".right-detail").hide();
      $(".design-area").css("width","calc(100% - 282px)");
      $('.logic-action-area .action-tabs-item').eq(0).trigger('click');
    ...
    showOperationArea: function(currField, cfg) {
      ...
      $operation.find('.title').html(currField.description||currField.name);
      $operation.show();
    },
    ...
    event2save: function() {
      ...
      me.hasChange = false;
      //当前设计页面是表单，则需要,更新业务字段列表
      if(me.data.id&& (me.formPageComponent.id == me.data.id)){
        //缓存当前表单和当前表单的业务控件
        me.cacheFormBizControls([{
          typeCode:'0',
          pageComponent:oui.clone(me.data)
        }]);
        PubSub.publish("com.startwe.saveForm.msg.show", {msgType:'page'}); //发布更新表单消息， 用于 业务字段的同步
      }
      var param = oui.getParam(); 
      if (param.id) {
        //加载表单定义
     ...
     * @param pageTypeTree
     */
    cacheFormBizControls:function (pageTypeTree){
      var me = this;
      var one = oui.findOneFromArrayBy(pageTypeTree||[],function (item){
        if(item.typeCode =='0'){
          return true;
        }
      });
      if(one){
        var page = one.pageComponent;
        ...
        me.formPageComponent = page;
        var arr =  oui.findManyFromArrayBy(controls,function (item){
          if(item.detailId){
            return false;
          }
          if(item.formField || item.isFormField){
        ...
        me.formBizControls = arr;

      }
    },

    loadPageDesign:function(pagedata) {
      ...
      var param = oui.getParam()    
      me.data.projectId = param.projectId
      me.data.circleId =  param.circleId
      me.data.pageId =  param.pageId

   
      

      me.paramCfg.page = data;
      me.saveUrl = oui.addParams(pagedata.saveurl,{
        projectId:me.data.projectId,
        circleId:me.data.circleId,
        pageId: me.data.pageId
      })
      me.data.id = pagedata.id


      //读取页面数据 请求search页面
      oui.postData(pagedata.searchUrl,
          {
            projectId:me.data.projectId,
            circleId:me.data.circleId,
            pageId: me.data.pageId  , 
            viewType:pagedata.typeCode?pagedata.typeCode:1
          },  (res)=> {
   
          var  newpage  = '';
          if(pagedata.typeCode==0){
            newpage = res.page;  
            
          }else{
            newpage = res.pageViews[0];
          }
          //处理 表单的业务字段处理
          //pageTypeTree
    ...
    //保存页面变量配置
    saveValSettingData(postdata){
 
      var me = this
      var saveUrl = postdata.saveUrl; //保存的api
      ...
      oui.postData(saveUrl, {page:params},  (res)=> {
    ...
    loadUrl:function (){
            ...
            var loadedBaseData = new Promise((resolve, reject) => {
             
            
              //读取远程树
            oui.postData( param.queryPageViewsByProjectIdUrl, {//模板数据
                  pageId:me.pageId,
                  moduleId:com.oui.absolute.AbsoluteDesign.data.moduleId,
                  viewType:Number(param.type),
                  projectId:param.projectId

              },  (res)=> {

                  me.cacheFormBizControls(res.pageTypeTree);
                  var pageTypeNames = oui.PageTypeEnum.findEnumNames();
                  oui.eachArray(pageTypeNames,function (name){ //处理所有呈现的路径
                    me[oui.PageTypeEnum[name].createPageViewUrlKey] = res[oui.PageTypeEnum[name].createPageViewUrlKey];
                  })
                  // me.createPageViewListUrl = res.createPageViewListUrl
                  me.queryCurrentListQueryLogicUrl = res.queryCurrentListQueryLogicUrl 
                  me.queryCurrentPageFieldsUrl = res.queryCurrentPageFieldsUrl 
                  me.queryLogicTreesByProjectUrl = res.queryLogicTreesByProjectUrl;
                  me.excutedSelectedLogicUrl = res.excutedSelectedLogicUrl  //数据源绑定 逻辑列表url
                  me.queryCurrentModelLogicUrl = res.queryCurrentModelLogicUrl  //交互设计中 逻辑选择列表 
                  me.getLogicByIdUrl = res.getLogicByIdUrl  //交互设计中 逻辑选择后 设置参数
                  // me.createPageViewEditUrl = res.createPageViewEditUrl  //交互设计中 逻辑选择后 设置参数
                  // me.createPageViewDetailUrl = res.createPageViewDetailUrl  //交互设计中 逻辑选择后 设置参数
                  me.data.appId = res.page.appId  //appid  
                  me.upload2AlibabaUrl = res.upload2AlibabaUrl //   
                  me.getLocalMapDataURl = res.getLocalMapDataURl //   定位 接口
                  
                  me.findPageViewUrl = res.findPageViewUrl //
                  me.findPageViewListUrl = res.findPageViewListUrl;//查询呈现页面地址

                  me.findFilesUrl = res.findFilesUrl //   
                  top.window.upload2AlibabaUrl = res.upload2AlibabaUrl;
                  me.findTableModelFieldsUrl = res.findTableModelFieldsUrl;//查询业务模型的字段列表
                  oui.eachTreeArray(res.tableModelTreeData||[],function (item){
                    item.value = item.id;
                    item.display =item.name;
                  });
                  me.tableModelTreeData = res.tableModelTreeData;

                  $('.project-name').text(res.projectName);
                  $('.page-name').text(res.page.name)

                  let nodeTreeData =res.pageTypeTree; 
                  let pageTypeTree = []; 

                  console.log('后台响应的 页面树数据',nodeTreeData);
                  if (pageTypeTree && nodeTreeData.length>0) {
                    ...
                    nodeTreeData.map((item, index) => {
                      ...
                      //表单页默认子级
                      if (item.hasOwnProperty('pageComponent')) {
                        children.push({
                          label: item.pageComponent.name,
                          id:  item.pageComponent.id,
                          value:item.pageComponent.id,
                          display:item.pageComponent.name,
                          pid:item.id,
                          class: "levelsTwo",
                          icon: "s-icon-page",
                          isLeaf: true,
                          showBtn:false,
                          typeCode:0,
                          renameUrl:item.pageComponent.extraAttrs.renameUrl,
                          searchUrl:param.queryPageViewsByProjectIdUrl,
                          saveurl:item.pageComponent.extraAttrs.savePageDesignUrl,
                        });

                      }

                      //如果有子级页面 
                      if (item.pageViewBeans && (item.pageViewBeans.length > 0)) {
                        item.pageViewBeans.map((v, k) => {
                          ...
                          children.push({
                            ...
                            class: "levelsTwo",
                            icon: "s-icon-page",
                            removeUrl: v.removeUrl,
                            renameUrl: v.renameUrl,
                            searchUrl: v.searchUrl,
                            saveurl:item.savePageViewUrl,
                            typeCode:item.typeCode,
                            isLeaf: true,
                            showBtn:false
                          });
            ...
            me.loadedBaseData = loadedBaseData
